...
const RAW_COURSES_DATA: CourseItem[] = [
  {
    ...
    topic:
      "¿Qué es un algoritmo, por qué importa la complejidad y cómo escala el costo computacional?",
    cormenChapter:
      "Capítulo 1: El papel de los algoritmos en la informática (CLRS 4ta Ed., págs. 5–15)",
    durationMinutes: 60,
    summary:
      "En una frase: Un algoritmo eficiente transforma la potencia computacional bruta en rendimiento real escalable, superando siempre al hardware rápido cuando los datos crecen.\n\nIntroducción a la algorítmica como tecnología central. Diferenciación entre hardware rápido y algoritmos eficientes con impacto empírico en escalabilidad masiva.",
    visualizerType: "big_o_chart",
    nextItemId: "clase-2",
    theoryContent: `# Clase 1: El Papel de los Algoritmos en la Computación

---

## 1. INTRODUCCIÓN Y MOTIVACIÓN

¿Por qué importa estudiar algoritmos? En la era de los procesadores multinúcleo a gigahertz y el *Big Data*, existe la falsa creencia de que la potencia bruta del hardware puede compensar el código ineficiente. Sin embargo, cuando el volumen de datos de entrada $n$ pasa de miles a millones, la **complejidad algorítmica** domina por completo sobre la velocidad de reloj del procesador. 

Un algoritmo no es simplemente "código que funciona". Es una especificación matemática rigurosa que transforma recursos computacionales finitos (tiempo de CPU y memoria RAM) en soluciones exactas. Un algoritmo ineficiente ejecutado en una supercomputadora colapsará irremisiblemente ante un algoritmo eficiente ejecutado en un teléfono móvil, siempre y cuando $n$ crezca lo suficiente.

**Breve Contexto Histórico:**
* **1843 – Ada Lovelace**: Escribió el primer algoritmo formal destinado a una máquina (la Máquina Analítica de Babbage).
* **1936 – Alan Turing**: Formalizó la definición matemática de "computable" mediante la Máquina de Turing, estableciendo los límites físicos del software.
* **1990 – Cormen, Leiserson, Rivest y Stein (CLRS)**: Publicaron *Introduction to Algorithms*, estandarizando el análisis asintótico como el lenguaje universal de los ingenieros de software de élite.

**Conexión:**
Esta clase establece la piedra angular del curso. Todas las estructuras avanzadas que estudiaremos después (árboles Red-Black, programación dinámica, grafos) persiguen un único objetivo fundamental: aplastar la curva de crecimiento temporal que veremos hoy.
...
Según **Cormen et al.**, un **algoritmo** es cualquier procedimiento computacional bien definido que toma algún valor (o conjunto) como **entrada** y produce algún valor como **salida**, satisfaciendo una relación funcional especificada.

Es vital separar dos conceptos que a menudo se confunden:
1. **Problema Computacional**: La declaración abstracta y matemática de la relación deseada entre entradas permitidas y salidas esperadas. (Ej: "Ordenar una lista de números").
2. **Algoritmo**: La secuencia finita, determinista e inequívoca de pasos concretos en código que resuelve dicho problema. (Ej: "Merge Sort" o "Insertion Sort").

Para que un algoritmo sea considerado correcto, debe gozar de tres propiedades inquebrantables:
* **Finitud**: Debe detenerse tras un número finito de operaciones.
* **Definibilidad**: Ningún paso puede ser ambiguo.
* **Correctitud**: Para toda entrada válida, debe detenerse y producir la salida exigida por el problema computacional. Un algoritmo incorrecto podría no detenerse (bucle infinito) o entregar respuestas erróneas.

### 2.2 Ejemplo Guiado Paso a Paso: El Problema de la Clasificación
Consideremos el problema de **ordenamiento**:
* **Entrada**: Una secuencia $I = \langle 31, 41, 59, 26, 41, 58 \rangle$.
* **Salida**: Una permutación ordenada $O = \langle 26, 31, 41, 41, 58, 59 \rangle$.

**Insertion Sort (Enfoque Cuadrático):**
Compara cada número con los anteriores y lo arrastra hasta su lugar. Si evaluamos $26$, lo compara con $59$, $41$ y $31$, realizando múltiples intercambios redundantes. El esfuerzo total crece como un cuadrado ($n^2$).

**Merge Sort (Enfoque Divide y Vencerás):**
Parte la lista a la mitad constantemente hasta tener listas de 1 elemento, y luego las unifica linealmente. En lugar de comparar todos contra todos, aprovecha que las sublistas ya vienen ordenadas al fusionarse. 

### 2.3 Errores Comunes de los Estudiantes
- **Confundir tiempo medido (\`clock()\`) con complejidad**: El tiempo en segundos varía por la temperatura de la CPU, otros procesos en segundo plano o el lenguaje. La complejidad mide la tasa de crecimiento de las instrucciones elementales, independientemente del hardware.
- **Ignorar el "Peor Caso"**: Diseñar algoritmos basados en cómo rinden con datos ideales, ignorando qué ocurre cuando el usuario ingresa datos intencionalmente desordenados o degenerados.

---

## 3. ANÁLISIS DE COMPLEJIDAD DETALLADO

El análisis de algoritmos implica predecir los recursos (memoria, ancho de banda, pero primariamente tiempo de CPU) que el algoritmo requerirá.

### 3.1 Derivación de Complejidad
Para el algoritmo **Insertion Sort**, el número de comparaciones en el peor caso (arreglo invertido) forma una progresión aritmética:
$$ \sum_{i=1}^{n-1} i = \frac{n(n-1)}{2} = \frac{1}{2}n^2 - \frac{1}{2}n $$
Cuando $n$ es enorme, el término $-\frac{1}{2}n$ y la constante $\frac{1}{2}$ se vuelven insignificantes. La tasa de crecimiento está dominada puramente por $n^2$. Decimos que su complejidad es $O(n^2)$.

### 3.2 Tabla de Crecimiento y Comparación Empírica
Asumamos una CPU hipotética que ejecuta $10^6$ operaciones por segundo ($1\mu s$ por operación). Veamos la explosión combinatoria:

| $n$ (Entrada) | $O(1)$ | $O(\log_2 n)$ | $O(n)$ | $O(n \log_2 n)$ | $O(n^2)$ | $O(2^n)$ |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **10** | $1\mu s$ | $3\mu s$ | $10\mu s$ | $33\mu s$ | $100\mu s$ | $1 ms$ |
| **100** | $1\mu s$ | $6.6\mu s$ | $100\mu s$ | $664\mu s$ | $10 ms$ | $4 \times 10^{16}$ años |
| **1,000** | $1\mu s$ | $10\mu s$ | $1 ms$ | $10 ms$ | $1 s$ | **Incalculable** |
| **10,000** | $1\mu s$ | $13\mu s$ | $10 ms$ | $0.13 s$ | $1.6 min$ | **Imposible** |
| **100,000** | $1\mu s$ | $16\mu s$ | $0.1 s$ | $1.6 s$ | $2.7 hrs$ | **Imposible** |
| **1,000,000** | $1\mu s$ | $20\mu s$ | $1 s$ | $20 s$ | $11.5 d$ | **Imposible** |

*Incluso con una supercomputadora 1000 veces más rápida, un algoritmo $O(2^n)$ seguiría tardando billones de años para $n=100$. ¡El algoritmo vence al hardware!*

---

## 4. APLICACIONES EN EL MUNDO REAL

1. **Motores de Búsqueda (Google/Bing)**: Enrutar billones de consultas web requiere estructuras de índices invertidos $O(1)$ en lugar de buscar linealmente texto por texto $O(n)$.
2. **Criptografía (RSA)**: La seguridad de internet se basa en un asimetría de complejidad: encriptar es algorítmicamente rápido $O(n^3)$, pero desencriptar sin la clave (factorización de primos) requiere algoritmos exponenciales $O(2^n)$ intratables para la humanidad.
3. **Rutas GPS**: Calcular la ruta óptima de tu casa al trabajo usa algoritmos de grafos subcuadráticos. Si tuvieran que evaluar todas las rutas físicamente posibles, el GPS tardaría años en encender.

---

## 5. NOTAS DE IMPLEMENTACIÓN EN C

C es un lenguaje no gestionado de bajo nivel, lo que significa que **tú eres responsable de la máquina**:
* **Desbordamiento de Enteros (Overflow)**: Si $n$ representa la cantidad de nodos y $n=1,000,000$, un contador de operaciones cuadráticas requerirá $1,000,000,000,000$. Ese número no cabe en un \`int\` tradicional de 32 bits en C (máximo $\approx 2.1 \times 10^9$). Provocará *Overflow* y valores negativos, destrozando tu programa. Usa \`long long\`.
* **Punteros y Asignación Dinámica**: En C, crear sub-arreglos (como exige Merge Sort) implica invocar manual y repetidamente a \`malloc()\` y \`free()\`. Si olvidas un \`free()\`, generarás un *Memory Leak*, agotando la RAM física en segundos.

---

## 6. GLOSARIO DE TÉRMINOS DE LA CLASE

* **Algoritmo**: Secuencia inequívoca y finita de instrucciones para resolver un problema.
* **Problema Computacional**: Definición matemática formal de la entrada y salida deseada.
* **Correctitud**: Propiedad de un algoritmo que garantiza la detención y la respuesta exacta siempre.
* **Complejidad Temporal**: Tasa matemática de crecimiento del tiempo de ejecución en función de la entrada $n$.
* **Hardware vs Software**: Demostración de que las mejoras tecnológicas físicas nunca pueden superar a las deficiencias matemáticas del diseño del código.

---

### Para profundizar
*Revisa el Capítulo 1, secciones 1.1 y 1.2 del libro "Introduction to Algorithms" (CLRS) para la prueba formal del enfrentamiento entre Insertion Sort en una supercomputadora vs Merge Sort en una PC lenta.*

### Guía de Animación
*A continuación verás una gráfica interactiva de Complejidad Big-O. Intenta activar las curvas de $O(n^2)$ y $O(2^n)$ al mismo tiempo y observa cómo la curva exponencial "desaparece" de la pantalla casi instantáneamente, demostrando visualmente por qué los algoritmos de fuerza bruta colapsan tu sistema.*
\`,
    checkQuestions: [
      {
        id: "clrs-1-q1",
        question:
          "¿Por qué un algoritmo O(n²) en la computadora más rápida del mundo siempre perderá a largo plazo contra un algoritmo O(n log n) en una computadora muy lenta?",
        options: [
          "Porque el lenguaje C es más rápido que Python.",
          "Porque los compiladores modernos optimizan el logaritmo.",
          "Porque la tasa matemática de crecimiento de n² superará inevitablemente cualquier constante de velocidad física a medida que n tienda a infinito.",
          "Es falso, una computadora cuántica siempre ganará sin importar la complejidad.",
        ],
        correctIndex: 2,
        explanation:
          "La complejidad asintótica dicta el comportamiento cuando el tamaño de la entrada crece hacia el infinito. Una constante de hardware será eventualmente opacada por la explosión cuadrática de n².",
      },
      {
        id: "clrs-1-q2",
        question:
          '¿Cuáles son las tres propiedades inquebrantables para que un procedimiento sea considerado un "algoritmo correcto"?',
        options: [
          "Velocidad, Memoria y Escalabilidad",
          "Finitud, Definibilidad y Correctitud",
          "Compilación nativa, Punteros y Recursión",
          "Determinismo, Aleatoriedad y Convergencia",
        ],
        correctIndex: 1,
        explanation:
          "Debe detenerse (Finitud), no tener pasos ambiguos (Definibilidad) y entregar siempre la salida esperada por la especificación del problema (Correctitud).",
      },
      {
        id: "clrs-1-q3",
        question:
          "Si mides tu algoritmo con \`clock()\` en C y demora 2 segundos, ¿has descubierto su complejidad temporal?",
        options: [
          "Sí, la complejidad temporal es exactamente 2 segundos.",
          "No, has medido el tiempo de reloj empírico, que depende del hardware actual, no la complejidad matemática asintótica.",
          "Sí, asumiendo que usas C, porque en Python daría un valor erróneo.",
          "No, porque \`clock()\` no cuenta el tiempo de disco duro.",
        ],
        correctIndex: 1,
        explanation:
          "El tiempo de reloj es anecdótico y varía. La complejidad mide cómo cambia el número de instrucciones lógicas a medida que n se incrementa.",
      },
      {
        id: "clrs-1-q4",
        question:
          "Según la tabla empírica mostrada, ¿qué sucede con un algoritmo O(2ⁿ) si n=100?",
        options: [
          "Tarda 100 milisegundos.",
          "Provoca un Memory Leak.",
          "El tiempo de ejecución excede drásticamente la edad del universo, haciéndolo computacionalmente imposible.",
          "Requiere dividir el arreglo en sublistas de log 100.",
        ],
        correctIndex: 2,
        explanation:
          "O(2^n) crece tan agresivamente que para 100 elementos, el número de operaciones supera 1.26 x 10^30, incalculable en la práctica física actual.",
      },
      {
        id: "clrs-1-q5",
        question: '¿Qué es un "Problema Computacional" según CLRS?',
        options: [
          "Un error en el compilador (Bug).",
          "Una declaración matemática que describe la relación deseada entre Entradas y Salidas.",
          "Un código en C mal estructurado.",
          "La imposibilidad de resolver P vs NP.",
        ],
        correctIndex: 1,
        explanation:
          "Un problema computacional es la abstracción teórica de lo que se busca resolver (ej. ordenar datos), mientras que el algoritmo es el mecanismo para lograrlo.",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Dados dos algoritmos A y B con complejidades f(n) = 100n y g(n) = n², determina para qué valor mínimo de n el algoritmo A comienza a ser estrictamente más eficiente que el algoritmo B.",
        ...
        explanation:
          "Para n <= 100, la constante 100 hace que g(n) = n² parezca más rápido o igual. Pero para n = 101 en adelante, g(n) crece como n² y f(n) = 100n demuestra su superioridad lineal.",
        testCases: [
          {
            id: "tc-1",
      ...
      {
        id: "clrs-1-ex2",
        title: "Nivel 1 (Conceptual): Predecir la Salida",
        description:
          "¿Qué propiedad vital rompe el siguiente pseudocódigo hipotético?\n\n1. Entra un número x\n2. Mientras x sea mayor que 0:\n3.    Haz algo útil\n4. Imprime x",
        cormenRef: "CLRS 1.1",
        initialCode:
          "// Modifica esta función para que retorne 1 si rompe la Finitud, 2 si rompe Definibilidad, 3 si Correctitud.\nint quePropiedadRompe() {\n    return 0;\n}",
        solutionCode: "int quePropiedadRompe() {\n    return 1; // Finitud\n}",
        hint: "Observa la línea 2. ¿Qué instrucción modifica a x para salir del bucle?",
        explanation:
          "El algoritmo rompe la propiedad de FINITUD. Dado que x nunca se decrece dentro del bucle, el programa caerá en un ciclo infinito y nunca se detendrá.",
        testCases: [
          {
            id: "tc-2",
            description: "Retornar propiedad violada",
            input: "",
            expectedOutput: "1",
          },
        ],
      ...
      {
        id: "clrs-1-ex3",
        title: "Nivel 2 (Aplicación Guiada): Completar Espacios O(n²)",
        description:
          "Considera un bucle en C que suma todos los elementos de una matriz NxN. Reemplaza los -1 para que se recorra todo correctamente, denotando un algoritmo O(n²).",
        cormenRef: "CLRS 1.2",
        initialCode:
          '#include <stdio.h>\n\nint sumMatrix(int N) {\n    int sum = 0;\n    for(int i = 0; i < -1; i++) {\n        for(int j = 0; j < -1; j++) {\n            sum += 1;\n        }\n    }\n    return sum;\n}\n\nint main() {\n    printf("%d", sumMatrix(10));\n    return 0;\n}',
        solutionCode:
          '#include <stdio.h>\n\nint sumMatrix(int N) {\n    int sum = 0;\n    for(int i = 0; i < N; i++) {\n        for(int j = 0; j < N; j++) {\n            sum += 1;\n        }\n    }\n    return sum;\n}\n\nint main() {\n    printf("%d", sumMatrix(10));\n    return 0;\n}',
        hint: "Una matriz cuadrada tiene N filas y N columnas. Reemplaza ambos -1 por N.",
        explanation:
          "Ambos bucles deben iterar N veces. Esto genera N iteraciones exteriores, y cada una detona N iteraciones interiores. Operaciones totales: N * N = N².",
        testCases: [
          {
            id: "tc-3",
            description: "Completar límites",
            input: "",
            expectedOutput: "100",
          },
        ],
      ...
      {
        id: "clrs-1-ex4",
        title: "Nivel 2 (Aplicación Guiada): Encontrar el Error (Bug)",
        description:
          "Un ingeniero hizo este código para predecir las operaciones de un O(n²). ¿Por qué fallará si N es 100,000? Corrige el código.",
        cormenRef: "CLRS 1.2",
        initialCode:
          '#include <stdio.h>\n\nint main() {\n    int n = 100000;\n    int ops = n * n;\n    printf("%d", ops);\n    return 0;\n}',
        solutionCode:
          '#include <stdio.h>\n\nint main() {\n    int n = 100000;\n    long long ops = (long long)n * n;\n    printf("%lld", ops);\n    return 0;\n}',
        hint: "100,000 multiplicado por 100,000 es 10,000,000,000. Ese número no cabe en un int de 32 bits.",
        explanation:
          "Se produce Integer Overflow. En C, un int de 32 bits llega hasta ~2.14x10^9. Se debe hacer cast a long long para no perder información.",
        testCases: [
          {
            id: "tc-4",
            description: "Imprimir valor exacto",
            input: "",
      ...
      {
        id: "clrs-1-ex5",
        title: "Nivel 3 (Implementación): Medición Real",
        description:
          "Escribe un programa que cuente las operaciones de un triple bucle anidado (O(n^3)) para \`n = 1000\`. Declara un contador, súmalo en el bucle interior, e imprímelo al final.",
        cormenRef: "CLRS 1.2",
        initialCode:
          "#include <stdio.h>\n\nint main() {\n    int n = 1000;\n    // Tu código aquí\n    return 0;\n}",
        solutionCode:
          '#include <stdio.h>\n\nint main() {\n    int n = 1000;\n    long long ops = 0;\n    for (int i=0; i<n; i++) {\n        for (int j=0; j<n; j++) {\n            for (int k=0; k<n; k++) {\n                ops++;\n            }\n        }\n    }\n    printf("%lld", ops);\n    return 0;\n}',
        hint: "Tres bucles \`for\` anidados. No olvides que 1000^3 es un billón (10^9), que requiere long long.",
        explanation:
          "Esta implementación prueba empíricamente el volumen masivo de operaciones en O(n^3). 1000 elementos requieren 1,000,000,000 operaciones.",
        testCases: [
          {
            id: "tc-5",
            description: "Contar operaciones anidadas",
            input: "",
      ...
      {
        id: "clrs-1-ex6",
        title: "Nivel 4 (Análisis): Justificar Big-O",
        description:
          "Escribe una función que retorne el valor de K (donde K es la cantidad de iteraciones) del siguiente bucle, si N es 1024.\n\`for(int i = 1; i <= n; i *= 2)\`",
        cormenRef: "CLRS 1.2",
        initialCode:
          '#include <stdio.h>\n\nint main() {\n    int k = 0; // Cambia esto al valor matemático\n    printf("%d", k);\n    return 0;\n}',
        solutionCode:
          '#include <stdio.h>\n\nint main() {\n    int k = 11;\n    printf("%d", k);\n    return 0;\n}',
        hint: "i toma valores: 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024. ¡Cuéntalos!",
        explanation:
          "El bucle se ejecuta para i = 2^0, 2^1, ... hasta 2^10. Son exactamente 11 iteraciones. Es un ejemplo clásico de complejidad O(log n).",
        testCases: [
          {
            id: "tc-6",
      ...
      {
        id: "clrs-1-ex7",
        title: "Nivel 5 (Desafío Integrador Avanzado)",
        description:
          "Escribe un algoritmo O(n) para encontrar el elemento mayor en un arreglo de enteros. El programa recibirá N en STDIN, seguido de N enteros. Debe imprimir el mayor.",
        cormenRef: "CLRS 2.1",
        initialCode:
          '#include <stdio.h>\n\nint main() {\n    int n;\n    if (scanf("%d", &n) != 1) return 0;\n    // Continuar leyendo n elementos...\n    return 0;\n}',
        solutionCode:
          '#include <stdio.h>\n\nint main() {\n    int n;\n    if (scanf("%d", &n) != 1) return 0;\n    if (n <= 0) return 0;\n    long long max_val = -99999999999;\n    for (int i = 0; i < n; i++) {\n        long long curr;\n        scanf("%lld", &curr);\n        if (curr > max_val) max_val = curr;\n    }\n    printf("%lld", max_val);\n    return 0;\n}',
        hint: "En lugar de comparar todos contra todos O(n²), mantén una variable \`max_val\` y actualízala leyendo cada número en un solo bucle O(n).",
        explanation:
          'Este patrón es la base de reducir la complejidad de O(n²) a O(n). Mantener un registro del "rey de la colina" mientras recorres la lista elimina comparaciones redundantes.',
        testCases: [
          {
            id: "tc-7",
            description: "Prueba estándar",
            input: "5\n10 2 30 4 5",
            expectedOutput: "30",
          },
        ],
...

  {
    ...
    topic:
      "Variables, tipos de datos, estructuras de control, funciones y la sintaxis formal de pseudocódigo en Cormen",
    cormenChapter: "Capítulo 2: Primeros Pasos (Getting Started - Sec. 2.1)",
    durationMinutes: 50,
    summary:
      "Análisis estructural del pseudocódigo formal de Cormen: bloques por identación, convenciones de arreglos base-1, paso por referencia y demostración de corrección mediante Invariantes de Bucle.",
    theoryContent: \`
## 1. INTRODUCCIÓN Y MOTIVACIÓN
...
\`,
    ...
    checkQuestions: [
      {
        id: "q2-1",
        question:
          "En el pseudocódigo estándar del libro de Cormen (CLRS), ¿en qué número de índice comienzan usualmente los arreglos?",
        options: [
          "En índice 0",
          "En índice 1",
          "En índice -1",
        ...
        correctIndex: 1,
        explanation:
          "¡Correcto! Cormen adopta la convención matemática donde los arreglos van de A[1] a A[n]. Al programar en C debemos traducirlo a base 0.",
        analogousExplanation:
          "En matemática tradicional contamos del 1 al 10. En informática física (C/JS) contamos desde el desplazamiento cero (offset 0).",
      },
      {
        id: "q2-2",
        question:
          "¿Qué tres propiedades deben verificarse para demostrar la corrección de un algoritmo mediante una Invariante de Bucle?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Exacto! Según CLRS Cap. 2.1, la prueba por Invariante requiere demostrar Inicialización (antes del bucle), Mantenimiento (entre iteraciones) y Terminación (al salir).",
        analogousExplanation:
          "Es análogo al principio de inducción matemática: caso base (inicialización), paso inductivo (mantenimiento) y conclusión (terminación).",
      },
      {
        id: "q2-3",
        question:
          "En el pseudocódigo de Cormen, ¿cómo se representa formalmente la operación de asignación de un valor a una variable (especialmente en sus versiones clásicas)?",
        options: ["x = y", "x <- y", "x := y", "x == y"],
        correctIndex: 1,
        explanation:
          "¡Correcto! En el pseudocódigo clásico de Cormen se utiliza la flecha (<-) para representar la asignación, evitando así la confusión con el operador de igualdad matemática (=).",
        analogousExplanation:
          'Imagina que tomas el valor de la derecha y lo "empujas" hacia adentro de la caja de la izquierda. (Nota: en ediciones recientes de CLRS se adoptó el = por influencia de C/Java).',
      },
      {
        id: "q2-4",
        question:
          'Al traducir el bloque "for i = 1 to n" del pseudocódigo de Cormen a C, ¿cómo se debe escribir la declaración del bucle asumiendo índices base-0?',
        options: [
          "for (int i = 1; i <= n; i++)",
          "for (int i = 0; i < n; i++)",
          "for (int i = 0; i <= n; i++)",
        ...
        correctIndex: 1,
        explanation:
          '¡Exacto! En C usamos base-0, así que empezamos en 0 y terminamos un elemento antes del tamaño total (i < n). El bucle "for i = 1 to n" ejecuta n veces.',
        analogousExplanation:
          'Si vas a subir 10 pisos, puedes contar "del piso 1 al piso 10" (Cormen) o "del piso 0 al piso 9" (C). En ambos casos subiste 10 pisos exactamente.',
      },
      {
        id: "q2-5",
        question:
          "En el pseudocódigo de Cormen, si un bucle debe iterar en orden descendente, ¿qué palabra clave se utiliza?",
        options: ["descending", "step -1", "downto", "reversed"],
        correctIndex: 2,
        explanation:
          '¡Correcto! CLRS utiliza la palabra clave "downto" para indicar que el índice decrece en cada iteración (ej. for i = n downto 1).',
        analogousExplanation:
          'Es como una cuenta regresiva de despegue: 10, 9, 8... "down to" (bajando hasta) 1.',
      },
    ],
    exercises: [
      {
        ...
        description:
          "En el pseudocódigo de Cormen, el último elemento de un arreglo de tamaño n está en A[n]. Escribe una función en C \`int obtenerUltimoElemento(int A[], int n)\` que devuelva correctamente el último elemento en C.",
        ...
        explanation:
          'En C, acceder a \`A[n]\` es un error de "Off-By-One" que lee memoria no asignada. La última posición válida es siempre \`A[n - 1]\`.',
      },
      {
        id: "ex-2-niv2-bug",
        title:
          'Nivel 2 (Aplicación Guiada): Corrección de Bug "Off-By-One" en Búsqueda Lineal',
        description:
          "El siguiente código intenta implementar la Búsqueda Lineal de Cormen en C, pero tiene un error de límite en el bucle for que causa un Segmentation Fault. Corrígelo.",
        ...
        testCases: [
          {
            id: "tc-2-2a",
            description:
              "Buscar x=30 en [10, 20, 30, 40], n=4 (Índice esperado: 2)",
          ...
          },
          {
            id: "tc-2-2b",
            description:
              "Buscar x=99 en [10, 20, 30, 40], n=4 (No encontrado: -1)",
            input: "[10, 20, 30, 40], 4, 99",
            expectedOutput: "-1",
          },
        ],
        explanation:
          "Cambiar \`i <= n\` a \`i < n\` asegura que el bucle se detenga en $i = n-1$, inspeccionando exactamente todos los elementos sin provocar desbordamiento de memoria.",
      },
      {
        id: "ex-2-niv3-impl",
        title:
          "Nivel 3 (Implementación C): Suma Acumulativa de Elementos Pares",
        description:
          "Escribe una función C \`int sumarElementosPares(int A[], int n)\` que recorra un arreglo de enteros y devuelva únicamente la suma acumulada de los elementos cuyo valor sea un número par (\`val % 2 == 0\`).",
        ...
        explanation:
          "El bucle evalúa la condición de paridad mediante el operador residuo \`% 2\`. Mantiene la invariante acumulando solo los valores válidos en $O(n)$ tiempo.",
      },
      {
        id: "ex-2-niv4-analisis",
        title:
          "Nivel 4 (Análisis): Verificador de Arreglo Estrictamente Monótono Creciente",
        description:
          "Escribe una función C \`int esMonotonoCreciente(int A[], int n)\` que devuelva \`1\` si para todo i se cumple A[i] < A[i+1], o \`0\` si existe alguna violación.",
        ...
        explanation:
          "La función utiliza cortocircuito para retornar \`0\` apenas se detecta la primera violación. La invariante de bucle garantiza que todo el subarreglo previo $A[0 \\\dots i]$ ha sido verificado.",
      },
      {
        id: "ex-2-niv5-desafio",
        title:
          "Nivel 5 (Desafío Avanzado Integrador): Algoritmo de Rotación de Arreglo in-place",
        description:
          "Escribe una función C \`void rotarIzquierda(int A[], int n, int k)\` que rote un arreglo k posiciones a la izquierda con complejidad espacial auxiliar O(1). [Marcado como Avanzado]",
        ...
        testCases: [
          {
            id: "tc-2-5",
            description:
              "Rotar [1, 2, 3, 4, 5] k=2 posiciones -> [3, 4, 5, 1, 2]",
            input: "[1, 2, 3, 4, 5], 5, 2",
            expectedOutput: "undefined",
          },
        ],
        explanation:
          "El célebre algoritmo de rotación mediante tres inversiones invierte bloques contiguos de memoria en $O(n)$ tiempo total sin utilizar memoria auxiliar adicional ($O(1)$ espacio).",
      },
    ],
    prevItemId: "clase-1",
    nextItemId: "clase-3",
...

  {
    ...
    type: "class",
    title:
      "Clase 3 – Punteros, Direcciones de Memoria y Structs en C para Cormen",
    topic:
      "Direccionamiento de memoria RAM (&), desreferenciación (*), la arimética de punteros y estructuras compuestas (structs)",
    cormenChapter:
      "Capítulo 10: Estructuras de Datos Elementales (Sec. 10.3 - Punteros y Objetos)",
    durationMinutes: 55,
    summary:
      "Aprender a leer y manipular la memoria física de la computadora: operadores &, *, paso de parámetros por referencia, aritmética de punteros, estructuras compuestas (struct) y el operador flecha (->).",
    theoryContent: \`
## 1. INTRODUCCIÓN Y MOTIVACIÓN
...
\`,
    ...
    checkQuestions: [
      {
        id: "q3-1",
        question:
          "En C, si tenemos \`int a = 50; int *ptr = &a;\`, ¿qué hace exactamente la instrucción \`*ptr = 99;\`?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Exacto! El operador \`*\` (desreferenciación) accede a la celda de memoria cuya dirección está guardada en \`ptr\` (es decir, la variable \`a\`) y reemplaza su contenido por 99.",
        analogousExplanation:
          "Si \`ptr\` es el número de la habitación 104, \`*ptr = 99\` entra a la habitación 104 y cambia la cama por una nueva.",
      },
      {
        id: "q3-2",
        question:
          "Dada una estructura \`struct Nodo { int clave; struct Nodo *next; } *p;\`, ¿cuál es la sintaxis correcta equivalente a \`(*p).clave\`?",
        options: ["p.clave", "p->clave", "p&*clave", "p..clave"],
        correctIndex: 1,
        explanation:
          "¡Correcto! En C, el operador flecha \`p->clave\` es el atajo oficial para desreferenciar un puntero a estructura y acceder a uno de sus campos.",
        analogousExplanation:
          "Es simplemente una forma abreviada y limpia de escribir \`(*p).clave\` sin tener que llenar el código de paréntesis extra.",
      },
      {
        id: "q3-3",
        question:
          "En C, si \`arr\` es un arreglo de enteros, ¿qué significa exactamente la expresión \`*(arr + i)\`?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Correcto! La notación de arreglos \`arr[i]\` es internamente convertida por C a aritmética de punteros: \`*(arr + i)\`. Al sumar i al puntero arr, se avanza exactamente i veces el tamaño del tipo de dato en memoria.",
        analogousExplanation:
          "Si la calle comienza en la casa 0 (\`arr\`) y caminas i casas hacia adelante (\`arr + i\`), luego abres la puerta (\`*\`) para ver qué hay adentro.",
      },
      {
        id: "q3-4",
        question:
          '¿Cuál es la consecuencia directa de pasar un \`struct\` muy grande "por valor" a una función (ej. \`void procesar(struct Datos d)\`) en lugar de usar un puntero?',
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Exacto! El paso por valor obliga a realizar una copia completa de toda la estructura. Si el struct pesa 1MB, se copiará 1MB cada vez que se llame a la función. Al pasarlo por puntero (\`struct Datos *d\`), solo se copian 8 bytes (la dirección).",
        analogousExplanation:
          'Si alguien te pide leer un libro de 1000 páginas, pasar "por valor" es imprimirle una copia completa de todo el libro. Pasar "por puntero" es simplemente darle la tarjeta con la ubicación de la biblioteca.',
      },
      {
        id: "q3-5",
        question:
          "¿Qué sucede a nivel de sistema si intentas desreferenciar un puntero que fue declarado pero no inicializado, o que apunta a \`NULL\`?",
        options: [
        ...
        correctIndex: 3,
        explanation:
          "¡Correcto! C no tiene un entorno seguro o máquina virtual (como Java/Python) que intercepte esto suavemente. El sistema operativo detecta el intento de acceso a memoria inválida o protegida y aborta el programa inmediatamente.",
        analogousExplanation:
          "Es como intentar abrir una puerta con una llave falsa en un edificio de alta seguridad: saltan las alarmas y el sistema te expulsa al instante.",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Escribe una función en C \`void duplicarValor(int *p)\` que tome un puntero a un entero \`p\` y duplique el valor alojado en esa dirección de memoria.",
        ...
        testCases: [
          {
            id: "tc-3-1",
            description:
              "Duplicar valor original val=21 (Resultado esperado: 42)",
            input: "21",
            expectedOutput: "undefined",
          },
        ],
        explanation:
          "Al desreferenciar \`*p\`, modificamos directamente la variable en el scope invocador sin necesidad de retornar un nuevo valor.",
      },
      {
        id: "ex-3-niv2-bug",
        title:
          "Nivel 2 (Aplicación Guiada): Intercambio de Variables (Swap) con Punteros",
        description:
          "Un estudiante escribió la función \`swap(int a, int b)\` intentando intercambiar dos variables, pero descubrió que fuera de la función no cambiaban. Corrige la firma y el cuerpo usando punteros \`int *a, int *b\`.",
        ...
        explanation:
          "Pasar direcciones de memoria \`&a\` y \`&b\` permite a la función modificar el estado en el frame de pila del llamador.",
      },
      {
        id: "ex-3-niv3-impl",
        title: "Nivel 3 (Implementación C): Modificador de Estructura de Nodo",
        description:
          "Dada la estructura \`struct Elemento { int id; int valor; };\`, escribe una función C \`void actualizarElemento(struct Elemento *elem, int nuevoId, int nuevoValor)\` que actualice ambos campos usando el operador flecha \`->\`.",
        ...
        explanation:
          "El operador \`->\` desreferencia la dirección del struct y modifica directamente los miembros de la estructura original.",
      },
      {
        id: "ex-3-niv4-analisis",
        title:
          "Nivel 4 (Análisis): Búsqueda de Mínimo y Máximo con Retorno Multivalor por Puntero",
        description:
          "En C una función solo puede retornar un único valor directamente. Implementa \`void obtenerMinMax(int A[], int n, int *minVal, int *maxVal)\` que devuelva el mínimo y el máximo de un arreglo escribiendo en los punteros provistos.",
        ...
        explanation:
          "El patrón de pasar punteros de salida (\`int *out\`) es el mecanismo idiomático en C para retornar múltiples resultados desde una sola función.",
      },
      {
        id: "ex-3-niv5-desafio",
        title:
          "Nivel 5 (Desafío Avanzado Integrador): Simulador de Asignador de Memoria Fija (Arena Allocator)",
        description:
          "Implementa una estructura \`struct Arena\` que administre un arreglo buffer de enteros de tamaño fijo (ej. 100 enteros). Escribe la función \`int* arenaAlloc(struct Arena *a, int tamano)\` que devuelva un puntero al bloque asignado incrementando el desplazamiento (offset) interno. [Marcado como Avanzado]",
        ...
        explanation:
          "Los asignadores por arena (Arena Allocators) son patrones de altísimo rendimiento usados en compiladores y motores de juegos para evitar la fragmentación de memoria de \`malloc\`.",
      },
    ],
    prevItemId: "clase-2",
    nextItemId: "clase-4",
...

  {
    ...
    topic:
      "Listas enlazadas simples y dobles, inserciones, punteros y fugas de memoria",
    cormenChapter: "Capítulo 10.2: Listas enlazadas (Linked Lists)",
    durationMinutes: 60,
    summary:
      "Construcción de estructuras dinámicas de datos: nodos, punteros head y next, inserción O(1), eliminación, listas circulares y doblemente enlazadas.",
    theoryContent: \`
## 1. INTRODUCCIÓN Y MOTIVACIÓN
...
*   [Visualgo: Animación de Inserciones y Eliminaciones](https://visualgo.net/en/list)
    ...
    checkQuestions: [
      {
        id: "q4-1",
        question:
          "¿Cuál es la ventaja fundamental de insertar un nuevo elemento al INICIO de una lista enlazada frente a un arreglo estático contiguo?",
        options: [
        ...
        correctIndex: 0,
        explanation:
          "¡Excelente! Insertar al frente de una lista enlazada requiere simplemente crear el nodo, conectarlo a head y reasignar head = nuevoNodo, operación constante O(1).",
        analogousExplanation:
          "Imagina una locomotora: enganchar un vagón nuevo justo al frente detrás de la máquina es instantáneo. En una fila fija de sillas de cine, tendrías que pedirle a todos que se corran una silla a la derecha.",
      },
      {
        id: "q4-2",
        question:
          "¿Por qué NO es posible implementar Búsqueda Binaria en tiempo O(log n) sobre una lista enlazada simple, incluso si los elementos están perfectamente ordenados?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Correcto! La búsqueda binaria fundamenta su velocidad logarítmica en poder acceder al índice (min+max)/2 en tiempo constante O(1). Al no tener acceso aleatorio, una lista enlazada obliga a saltar O(n) veces para llegar a la mitad.",
        analogousExplanation:
          "Si tienes un libro impreso (Arreglo), abrirlo justo por la mitad es instantáneo. Si tienes un rollo de papiro antiguo (Lista), para ver la mitad debes desenrollarlo manualmente desde el inicio.",
      },
      {
        id: "q4-3",
        question:
          "Supongamos que conoces exactamente la dirección de memoria de un nodo M (que está en medio de la estructura) y quieres eliminarlo. ¿Por qué una Lista Doblemente Enlazada lo hace en O(1) mientras que la Simple cuesta O(n)?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          '¡Exacto! Para borrar un nodo necesitas enlazar a su predecesor con su sucesor. En la lista simple, el predecesor solo se encuentra buscándolo desde el principio O(n). El puntero "prev" de la lista doble te lo da en O(1).',
        analogousExplanation:
          "Es como estar en el tercer piso de un edificio sin escaleras de bajada (lista simple): para ir al piso dos, debes tirarte, volver al lobby y subir dos pisos. Con escaleras de bajada (puntero prev), es un paso.",
      },
      {
        id: "q4-4",
        question:
          '¿Qué es un "memory leak" (fuga de memoria) en el contexto de listas enlazadas en C?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Correcto! Una fuga de memoria ocurre cuando pierdes la dirección de un bloque alojado dinámicamente con malloc() antes de llamar a free(), robando RAM permanentemente hasta que el programa se cierra.",
        analogousExplanation:
          "Imagínate aparcar tu auto en un parking gigante (RAM), perder la llave y el ticket (Puntero), e irte. Tu auto quedará ocupando esa plaza para siempre y nadie más podrá usarla.",
      },
      {
        id: "q4-5",
        question:
          'Si quieres avanzar la "cabeza" de la lista y eliminar el nodo antiguo, ¿cuál es el orden correcto y seguro para evitar un Segmentation Fault?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Exacto! Primero respaldas la dirección actual en un puntero temporal, luego avanzas la cabeza de forma segura usando la memoria válida, y finalmente haces free del temporal respaldado.",
        analogousExplanation:
          "Regla de Tarzán: no puedes soltar la liana actual (free) antes de haber agarrado firmemente la siguiente liana (head = head->next).",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Dada la estructura \`struct Nodo { int dato; struct Nodo *siguiente; };\`, escribe una función \`int obtenerPrimerValor(struct Nodo *cabeza)\` que devuelva el campo \`dato\` del nodo cabeza o \`-1\` si la lista es nula (\`NULL\`).",
        ...
        explanation:
          "El acceso a campos de una estructura mediante un puntero se realiza con el operador flecha \`->\`, verificando siempre que el puntero no sea nulo.",
      },
    ],
    prevItemId: "clase-3",
    nextItemId: "clase-5",
...

  {
    ...
    topic:
      "Casos base y recursivos, la pila de llamadas (Call Stack), pensamiento recursivo y explosión exponencial",
    cormenChapter: "Capítulo 3 y 4.4: Recursividad y Árboles de Recursión",
    durationMinutes: 60,
    summary:
      "Comprender cómo una función se llama a sí misma dividiendo problemas en subproblemas idénticos más pequeños, trazando el Call Stack y analizando la complejidad O(2^n).",
    theoryContent: `
## 1. INTRODUCCIÓN Y MOTIVACIÓN
...
*   **Herramienta Visual**: Recomendamos fuertemente dibujar en un papel el árbol de llamadas paso a paso para cualquier función recursiva nueva que escribas.
    ...
    checkQuestions: [
      {
        id: "q5-1",
        question:
          "¿Qué sucede a nivel de memoria si escribimos una función recursiva en C y olvidamos colocar el Caso Base?",
        options: [
        ...
        correctIndex: 3,
        explanation:
          "¡Exacto! Cada llamada ocupa espacio en el Call Stack. Sin caso base, las llamadas se apilan infinitamente hasta desbordar la pila, causando un crash.",
        analogousExplanation:
          "Es como meter una caja dentro de otra caja infinitamente; eventualmente te quedarás sin cartón (memoria).",
      },
      {
        id: "q5-2",
        question:
          "¿Cuál es la complejidad de tiempo de una implementación recursiva ingenua de la serie de Fibonacci, $F(n) = F(n-1) + F(n-2)$?",
        options: ["O(1)", "O(n)", "O(n log n)", "O(2^n)"],
        correctIndex: 3,
        explanation:
          "¡Correcto! Es $O(2^n)$. Al llamar a la función dos veces por cada nivel, el árbol de llamadas se duplica en ancho en cada paso, generando una explosión exponencial de cálculos redundantes.",
        analogousExplanation:
          "Si un rumor lo cuentas a dos amigos, y cada uno a otros dos, la cantidad de gente hablando crece exponencialmente.",
      },
      {
        id: "q5-3",
        question:
          "En el cálculo del factorial recursivo, ¿por qué la complejidad de memoria (espacio) es $O(n)$ en lugar de $O(1)$?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          '¡Correcto! En la recursión profunda, las llamadas se "pausan" esperando la respuesta de sus sub-llamadas. Si $n=100$, habrá 100 marcos de pila abiertos simultáneamente.',
        analogousExplanation:
          'Es como tener 100 pestañas del navegador abiertas al mismo tiempo porque en cada una hiciste clic en "Abrir en nueva pestaña" antes de leerla.',
      },
      {
        id: "q5-4",
        question:
          '¿Qué característica define a la "Recursión de Cola" (Tail Recursion)?',
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Exacto! Si no hay operaciones pendientes (como una multiplicación multiplicando el resultado recursivo), la función no necesita guardar su estado local, lo que teóricamente permite al compilador optimizar la memoria a $O(1)$.",
        analogousExplanation:
          "Es como pasarle el testigo a otro corredor y salir de la pista de inmediato, en lugar de quedarte esperando a que termine para aplaudirle.",
      },
      {
        id: "q5-5",
        question:
          "Si \`int f(int x) { if(x == 0) return 0; return x + f(x-1); }\`, ¿qué devuelve \`f(3)\`?",
        options: ["3", "4", "6", "9"],
        correctIndex: 2,
        explanation:
          "¡Correcto! La traza es: $3 + f(2) \\rightarrow 3 + (2 + f(1)) \\rightarrow 3 + 2 + (1 + f(0)) \\rightarrow 3 + 2 + 1 + 0 = 6$.",
        analogousExplanation:
          "Es simplemente la suma de los números del 1 al $n$.",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Implementa la función en C \`int factorial(int n)\` que calcule el factorial de un número entero positivo de manera recursiva.",
        ...
        explanation:
          "Cada llamada recursiva apila un marco en la pila de llamadas de C hasta llegar al caso base $n \\\le 1$.",
      },
    ],
    prevItemId: "clase-4",
    nextItemId: "taller-1",
...

  {
    ...
    topic:
      "Resolución guiada de ejercicios intensivos: Listas enlazadas, arreglos y pensamiento recursivo",
    cormenChapter: "Capítulo 4: Divide y Vencerás & Problemas de Recursión",
    durationMinutes: 70,
    summary:
      "Taller práctico obligatorio: resolución paso a paso integrando listas enlazadas con recursión, conteo, sumas e inversión.",
...
`,
    ...
    checkQuestions: [
      {
        id: "qt1-1",
        question:
          "En el problema de las Torres de Hanói con $n$ discos, ¿cuál es el número mínimo exacto de movimientos necesarios?",
        options: ["$2^n - 1$", "$n^2$", "$n \\log n$", "$2n$"],
        correctIndex: 0,
        explanation:
          "¡Excelente! La recurrencia $T(n) = 2T(n-1) + 1$ se resuelve matemáticamente en $T(n) = 2^n - 1$ movimientos.",
        analogousExplanation:
          "Para 1 disco = 1 mov. Para 2 discos = 3 mov. Para 3 discos = 7 mov ($2^3 - 1 = 7$). Crece de forma exponencial.",
      },
      {
        id: "qt1-2",
        question:
          'Si resolvemos la "Longitud de una Lista Enlazada" recursivamente, ¿qué representa nuestro Caso Base?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Exacto! El final de la lista está marcado por el puntero NULL. Ese es nuestro estado vacío y trivial donde la longitud es 0.",
        analogousExplanation:
          "Es como caminar por una cadena humana; te detienes y cuentas 0 cuando llegas a la persona que no le está dando la mano a nadie más.",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Implementa la función recursiva en C \`int sumaRecursiva(int arr[], int n)\` que calcule la suma de los $n$ elementos de un arreglo sin usar bucles.",
        ...
        explanation:
          "La recursión divide el problema reduciendo el tamaño del arreglo en 1 en cada llamada.",
      },
      {
        id: "ex-t1-2",
        title: "Ejercicio Taller 1.2: Longitud de Lista Enlazada",
        description:
          "Dada la estructura \`struct Nodo { int dato; struct Nodo *siguiente; };\`, escribe la función recursiva \`int contarNodos(struct Nodo *head)\`.",
        ...
        testCases: [
          {
            ...
            input:
              "{ dato: 1, siguiente: { dato: 2, siguiente: { dato: 3, siguiente: null } } }",
            expectedOutput: "3",
          },
          {
            id: "t2",
        ...
        explanation:
          "Recorrer una lista enlazada recursivamente es la base para operaciones complejas como eliminarla o invertirla.",
      },
      {
        id: "ex-t1-3",
        title: "Ejercicio Taller 1.3: MCD (Algoritmo de Euclides)",
        description:
          "Implementa el algoritmo de Euclides recursivo \`int mcd(int a, int b)\`. Recordando que MCD(a, b) = MCD(b, a % b).",
        ...
        explanation:
          "El algoritmo de Euclides es uno de los usos más elegantes y eficientes de la recursión matemática.",
      },
    ],
    prevItemId: "clase-5",
    nextItemId: "clase-6",
  },
  {
    ...
    topic:
      "Árboles binarios, recorridos (Inorden, Preorden, Postorden) y Árboles Binarios de Búsqueda (BST)",
    cormenChapter:
      "Capítulo 12: Árboles Binarios de Búsqueda (Binary Search Trees)",
    durationMinutes: 60,
    summary:
      "Estructura jerárquica no lineal: nodos con hijos izquierdo y derecho, propiedad de búsqueda de un BST y recorridos de árbol.",
...
*   **Comunidad y Repositorios:** Implementaciones modulares de árboles en el repositorio clásico del Kernel de Linux (rbtrees - Red-Black Trees).`,
    ...
    checkQuestions: [
      {
        id: "q6-1",
        question:
          "Si ejecutamos un recorrido INORDEN (Inorder Traversal) sobre un Árbol Binario de Búsqueda (BST), ¿en qué secuencia se procesan las claves?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Excelente! Debido a la propiedad del BST ($Izquierdo \le Raíz \le Derecho$), el recorrido Inorden visita las claves ordenadas perfectamente de menor a mayor.",
        analogousExplanation:
          "Imagina una biblioteca ordenada por código: Inorden recorre primero el estante izquierdo (menores), luego la mesa central (raíz) y finalmente el estante derecho (mayores).",
      },
      {
        id: "q6-2",
        question:
          "En términos de terminología de árboles, ¿cómo se define un 'Nodo Hoja' (Leaf Node)?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "Correcto. Una hoja representa el punto terminal de un árbol. Al no tener ramificaciones adicionales, sus punteros `izquierdo` y `derecho` son `NULL`.",
        analogousExplanation:
          "Es el extremo literal de la rama de un árbol real: donde termina el crecimiento y no salen brotes nuevos.",
      },
      {
        id: "q6-3",
        question:
          "¿Cuál es el riesgo de complejidad temporal o 'Peor Caso' (Worst Case) al insertar una secuencia de números ya ordenados (ej. 1, 2, 3, 4, 5) en un BST simple?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Muy bien! Un BST simple no se balancea solo. Insertar datos ordenados crea ramas que solo van hacia la derecha, arruinando la propiedad de división dicotómica y escalando la búsqueda a $O(n)$.",
        analogousExplanation:
          "Es como intentar jugar al 'Adivina Quién' pero todas tus preguntas solo logran descartar a una persona a la vez, en lugar de dividir el tablero a la mitad.",
      },
      {
        id: "q6-4",
        question:
          "Si estás escribiendo una función recursiva para eliminar un árbol completo de la memoria liberando (free) sus nodos, ¿qué recorrido deberías usar?",
        options: [
          "Preorden (Preorder)",
          "Inorden (Inorder)",
          "Postorden (Postorder)",
        ...
        correctIndex: 2,
        explanation:
          "¡Exacto! Con Postorden primero liberas el subárbol izquierdo, luego el subárbol derecho, y solo al final liberas el nodo padre (Raíz). Si usaras Preorden, liberarías la raíz primero y perderías los punteros a sus hijos.",
        analogousExplanation:
          "Como desarmar una pirámide de vasos: si quitas la base primero (la raíz en Preorden), todo colapsa. Debes quitar los bordes exteriores (los hijos en Postorden) antes de quitar el centro.",
      },
    ],
    exercises: [
      {
        id: "ex-6",
        title:
          "Ejercicio 6: Búsqueda en un Árbol Binario de Búsqueda (BST) en C",
        description:
          "Dada la estructura `struct NodoBST { int valor; struct NodoBST *izq; struct NodoBST *der; };`, escribe la función `bool buscarBST(struct NodoBST *raiz, int x)` que devuelva `true` si `x` está en el BST o `false` en caso contrario.",
        ...
        testCases: [
          {
            ...
            input:
              "{ valor: 10, izq: null, der: { valor: 15, izq: null, der: null } }, 15",
            expectedOutput: "true",
          },
          {
            id: "t2",
        ...
        explanation:
          "Aprovecha la propiedad del BST: los valores menores están en el subárbol izquierdo y los mayores en el derecho, ofreciendo búsqueda en $O(h)$.",
      },
    ],
    prevItemId: "taller-1",
    nextItemId: "clase-7",
...

  {
    ...
    topic:
      "Sesión de consolidación de conceptos fundamentales antes del Análisis Asintótico",
    cormenChapter: "Consolidación de Capítulos 4, 10 y 12",
    durationMinutes: 50,
    summary:
      "Conectar las estructuras de datos dinámicas con el razonamiento recursivo. Desafíos integrados.",
...
*   **Práctica Complementaria:** Te sugerimos implementar, por tu cuenta, la inserción de un nodo en un árbol BST en C (combinación perfecta de punteros y recursión).`,
    ...
    checkQuestions: [
      {
        id: "q7-1",
        question:
          "¿Por qué la mayoría de las operaciones sobre árboles binarios se escriben de forma recursiva?",
        options: [
        ...
        correctIndex: 0,
        explanation:
          "¡Exacto! El caso base suele ser el árbol vacío (null) y el caso recursivo procesa la raíz y llama recursivamente a left y right.",
        analogousExplanation:
          "Un árbol botánico real está hecho de ramas: si cortas una rama principal, parece un árbol más pequeño completo.",
      },
      {
        id: "q7-2",
        question:
          "En términos estructurales, ¿cuál de las siguientes afirmaciones describe mejor a una Lista Enlazada Simplemente?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Correcto! Matemáticamente, una lista es un árbol donde todos los nodos solo tienen un subárbol, perdiendo la ramificación dicotómica pero reteniendo la estructura de punteros dependientes.",
        analogousExplanation:
          "Una lista enlazada es un árbol que creció como una palmera de un solo tronco alto, sin ramas a los lados.",
      },
      {
        id: "q7-3",
        question:
          "¿Cuál es el peligro principal de recorrer una Lista Enlazada de 1,000,000 de nodos utilizando recursión en C?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "Muy bien. Al apilar 1,000,000 de Stack Frames para las llamadas recursivas de la lista, la pila del SO estalla. Por eso preferimos bucles iterativos $O(1)$ en memoria para listas inmensamente largas.",
        analogousExplanation:
          'Es como pedirle a tu cerebro que memorice 1 millón de pasos para ir al baño antes de dar el primer paso; te agotarás mentalmente antes de arrancar. Mejor solo memorizar "el siguiente paso".',
      },
      {
        id: "q7-4",
        question:
          "Durante la recursión (por ejemplo al recorrer una lista), ¿cuándo se ejecuta el código que escribimos DESPUÉS de la llamada a sí misma?",
        options: [
          "Antes del caso base.",
        ...
        correctIndex: 3,
        explanation:
          'Correcto. Todo lo que va después de la llamada recursiva, es el "Procesamiento Post-Retorno" que opera una vez que el problema alcanzó el fondo (caso base) y rebota hacia atrás.',
        analogousExplanation:
          "Como recoger las migajas de pan que dejaste al entrar al bosque: solo las recoges en el camino de vuelta a casa.",
      },
      {
        id: "q7-5",
        question:
          "Si tienes 1,000,000 de usuarios ordenados por ID, ¿cuál es la diferencia clave entre buscarlos en una Lista Enlazada vs. un Árbol Binario de Búsqueda (BST) perfectamente balanceado?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Exacto! Aunque la lista esté ordenada, al ser secuencial debes iterar uno por uno ($O(n)$). El BST permite ramificar (dividir por dos) en cada paso, logrando $O(\\log n)$ ($2^{20} \\approx 1,000,000$).",
        analogousExplanation:
          "Buscar en la lista es leer un libro de 1 millón de páginas desde el principio. Buscar en el BST es abrir el libro a la mitad, ver si te pasaste, y repetir con la mitad restante (¡solo abres el libro 20 veces!).",
      },
      {
        id: "q7-6",
        question:
          'Al escribir una función recursiva pura en C, ¿por qué calcular la "longitud de una lista" y calcular la "altura de un árbol" comparten el mismo caso base estructural?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          '¡Excelente conexión! Ya sea `head == NULL` o `raiz == NULL`, el caso base para estructuras dinámicas siempre representa "llegamos a un vacío que tiene longitud/altura 0".',
        analogousExplanation:
          "Ambos son como medir un río: el caso base es cuando llegas al lugar exacto donde el río se seca y ya no hay agua.",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Escribe una función en C `int calcularAltura(struct NodoBST *raiz)` que calcule la altura de un árbol binario recursivamente (un árbol vacío `NULL` tiene altura 0).",
        ...
        testCases: [
          {
            ...
            input:
              "{ valor: 10, izq: { valor: 5, izq: null, der: null }, der: null }",
            expectedOutput: "2",
          },
          {
            id: "t2",
        ...
        explanation:
          "La altura se calcula desde las hojas hacia la raíz sumando 1 por cada nivel del camino más largo.",
      },
    ],
    prevItemId: "clase-6",
    nextItemId: "clase-8",
...

  {
    ...
    topic:
      "Crecimiento de funciones, Cotas Superiores/Inferiores (O, \Omega, \Theta) y Análisis Matemático de Algoritmos",
    cormenChapter: "Capítulo 3: Growth of Functions",
    durationMinutes: 75,
    summary:
      "Aprende a analizar el rendimiento de tus algoritmos con el rigor matemático de la Notación Asintótica, el lenguaje universal de la informática.",
...
*   **Referencia:** *Cormen (CLRS) Capítulo 3*. Contiene las propiedades de reflexividad y transitividad de estas cotas.`,
    ...
    checkQuestions: [
      {
        id: "q8-1",
        question:
          "Según la definición formal de Big-O ($O$), si decimos que el tiempo es $O(g(n))$, ¿qué estamos garantizando matemáticamente?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          '¡Exacto! La desigualdad $0 \\le f(n) \\le c \\cdot g(n)$ para $n \\ge n_0$ asegura que $g(n)$ actúa como un "techo" matemático (una cota superior).',
        analogousExplanation:
          'Es como decir "A partir del año que viene ($n_0$), mi gasto ($f$) jamás superará el doble ($c$) de tu salario ($g$)".',
      },
      {
        id: "q8-2",
        question:
          "Al demostrar que $3n^2 = O(n^3)$, ¿cuál es el propósito de las constantes $c$ y $n_0$?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Muy bien! $n_0$ define a partir de qué volumen de datos las gráficas se cruzan, y $c$ es un ajuste para acomodar factores pequeños iniciales.",
        analogousExplanation:
          "A un auto lento ($n^2$) se le da una ventaja, pero el auto rápido ($n^3$) viaja a una velocidad constante $c$. A partir de un kilómetro $n_0$, el auto rápido lo rebasará y jamás volverá a estar detrás.",
      },
      {
        id: "q8-3",
        question:
        ...
        correctIndex: 2,
        explanation:
          "¡Perfecto! El primer ciclo hace $n$ pasos, el segundo $n-1$, etc. Esto es una progresión aritmética cuya suma es $\\frac{n(n+1)}{2}$. Al ignorar factores constantes y términos menores ($n/2$), domina el término $O(n^2)$.",
        analogousExplanation:
          "Imagina lavar las ventanas de un rascacielos. Lavas todas las de tu piso y las de los pisos superiores. A medida que subes lavas menos, pero el total de ventanas lavadas crece de forma cuadrática respecto al número de pisos.",
      },
      {
        id: "q8-4",
        question:
          "Si un algoritmo tiene una función exacta de costo $f(n) = 15000n + 5n^2 + 888$, ¿cómo se expresa en Big-O puro?",
        options: ["$O(15000n)$", "$O(888)$", "$O(n^2)$", "$O(n)$"],
        correctIndex: 2,
        explanation:
          "¡Excelente! En análisis asintótico ($n \\to \\infty$), aplicamos la Regla del Polinomio: el término con el exponente mayor devora a todos los demás. Se ignora la gran constante 15000 y el factor aditivo 888.",
        analogousExplanation:
          "Si compraste la Luna, no te preocupas por un impuesto extra de $888 o por el flete local. El precio masivo a nivel intergaláctico (el $n^2$) ahoga cualquier otro gasto terrestre.",
      },
      {
        id: "q8-5",
        question:
          "¿Cuál es la diferencia estricta entre las notaciones Big-O ($O$) y Big-Omega ($\\Omega$)?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Correcto! $O(g(n))$ garantiza que el código nunca irá más lento que esa cota. $\\Omega(g(n))$ garantiza que jamás podrá ser más rápido que eso. Y $\\Theta(g(n))$ ocurre cuando ambas cotas coinciden.",
        analogousExplanation:
          'Big-O es decir "Llegaré a mi destino en máximo 1 hora". Big-Omega es decir "El tráfico está perfecto, pero físicamente tardaré al menos 30 min".',
      },
    ],
    exercises: [
      {
        ...
        description:
          "Escribe una función en C `int analizarMatriz(int n)` con dos bucles `for` anidados independientes (ambos de 0 a $n-1$) que incremente y devuelva un contador, evidenciando el crecimiento $O(n^2)$.",
        ...
        testCases: [
          ...
          },
          {
            id: "t2",
            description:
              "Para n = 10, las iteraciones crecen cuadráticamente a 10*10",
            input: "10",
            expectedOutput: "100",
          },
        ],
        explanation:
          "Dado que el bucle interno corre exactamente $n$ veces, por cada una de las $n$ iteraciones del externo, el total estricto es $n \\times n = n^2$. Por tanto, es $O(n^2)$ y $\\Theta(n^2)$ simultáneamente.",
      },
    ],
    prevItemId: "clase-7",
  },
  {
    ...
    topic:
      "Análisis de algoritmos recursivos, resolución de ecuaciones de recurrencia y el Teorema Maestro",
    cormenChapter: "Capítulo 4: Método Maestro y Árboles de Recurrencia",
    durationMinutes: 65,
    summary:
      "Aprender la herramienta matemática definitiva de Cormen para resolver la complejidad de algoritmos tipo Divide y Vencerás.",
...
*   **Advertencia:** El Teorema Maestro NO es universal. Si la diferencia entre $f(n)$ y $n^{\\log_b a}$ no es polinomial (por ejemplo, difieren solo por un factor de $\\log n$), el teorema estándar falla y debemos recurrir al método de sustitución avanzado.`,
    ...
    checkQuestions: [
      {
        id: "q9-1",
        question:
          "En la ecuación estructural $T(n) = aT(n/b) + f(n)$ del Teorema Maestro, ¿qué representa matemáticamente la variable $a$?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Exacto! La variable $a$ multiplica a la función $T$, indicando cuántas ramas hijas nacen de cada nodo del árbol de recursividad (cuántos subproblemas se generan).",
        analogousExplanation:
          "Si $a=2$, significa que cada vez que divides el problema, invocas 2 copias más pequeñas de ti mismo (como clonarse en 2 mini-versiones).",
      },
      {
        id: "q9-2",
        question:
          "Si resolvemos la recurrencia $T(n) = 4T(n/2) + n$, ¿cuál es el resultado y a qué caso del Teorema Maestro corresponde?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Muy bien! $a=4, b=2$. El costo de hojas es $n^{\\log_2 4} = n^2$. Como $f(n) = n$ es asintóticamente menor polinomialmente que $n^2$, aplica el Caso 1 (las hojas dominan). El resultado es $\\Theta(n^2)$.",
        analogousExplanation:
          "Crear 4 subproblemas partiendo a la mitad es demasiado explosivo. El trabajo de hacer tantas divisiones sepulta al simple esfuerzo lineal ($n$) de combinarlos.",
      },
      {
        id: "q9-3",
        question:
          "Para aplicar el Caso 3 (La raíz domina), se requiere que $f(n)$ sea mayor que $n^{\\log_b a}$ polinomialmente, Y además se debe cumplir una condición extra. ¿Cuál es?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Perfecto! La condición de regularidad garantiza que el trabajo real decrece constantemente a medida que bajamos por el árbol, asegurando que la raíz es verdaderamente el cuello de botella computacional dominante.",
        analogousExplanation:
          'Asegura que "el jefe" (la raíz) realmente trabaja más que "los empleados" (subproblemas) combinados. Si no, la jerarquía se rompe.',
      },
      {
        id: "q9-4",
        question:
          "¿Qué método general de resolución requiere proponer una complejidad inicial (adivinar) y luego demostrar formalmente que es cierta usando inducción matemática?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "Correcto. El Método de Sustitución (Substitution Method) asume una respuesta (ej. $T(n) \\le c \\cdot n^2$) y luego fuerza matemáticamente esa cota por inducción sobre la ecuación recursiva original.",
        analogousExplanation:
          "Como en un juicio: formulas la hipótesis (adivinas) y luego presentas pruebas irrefutables (inducción) para convencer al juez.",
      },
      {
        id: "q9-5",
        question:
          'En un algoritmo tipo "Divide y Vencerás" donde la recurrencia es $T(n) = 2T(n/2) + n$, ¿por qué el Teorema Maestro determina que la complejidad es $\\Theta(n \\log n)$?',
        options: [
        ...
        correctIndex: 1,
        explanation:
          '¡Excelente! En el Caso 2 hay un "empate" perfecto. Ningún nivel del árbol de ejecución pesa asintóticamente más que otro. Como cada nivel toma trabajo $n$, y hay $\\log_2 n$ niveles de altura, el total acumulado es $n \\cdot \\log n$.',
        analogousExplanation:
          "Es como un edificio donde cada piso tiene el mismo número de ladrillos. El peso total es (ladrillos por piso) $\\times$ (altura del edificio).",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Implementa la función en C `const char* resolverMetodoMaestro(double a, double b, double d)` que retorne el tiempo asintótico según los 3 casos del Método Maestro.",
        ...
        explanation:
          "El Método Maestro compara la tasa de división del trabajo $n^{\\log_b a}$ contra la función de combinación $f(n) = O(n^d)$.",
      },
    ],
    prevItemId: "clase-8",
    nextItemId: "taller-2",
  },
  {
    ...
    topic:
      "Identificación de complejidad algorítmica y resolución de recurrencias de Divide y Vencerás",
    cormenChapter: "Taller Práctico sobre Cap 3 y 4",
    durationMinutes: 45,
    summary:
      "Consolida tu habilidad para aplicar notación asintótica y el Método Maestro a problemas reales de código y ecuaciones recursivas.",
...
*   **Enlaces conceptuales:** Repasa la **Clase 8** para asentar las reglas empíricas de los bucles, y la **Clase 9** para las demostraciones formales de sustitución.`,
    ...
    checkQuestions: [
      {
        id: "qt2-1",
        question:
          "¿A qué caso del Método Maestro pertenece $T(n) = 2T(n/2) + O(n)$ y cuál es su solución?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Excelente! En esta recurrencia (típica de Merge Sort), $a=2, b=2$ y el costo crítico es $n^{\\log_2 2} = n$. Como $f(n) = O(n)$ crece a la misma tasa, es un empate (Caso 2) y agregamos el factor multiplicativo $\\log n$.",
        analogousExplanation:
          "Como en una empresa donde la carga de trabajo en la sede principal ($n$) pesa exactamente igual que la carga sumada de todas las sucursales hijas combinadas. Multiplicamos el peso de un nivel por la cantidad total de jerarquías ($\\log n$).",
      },
      {
        id: "qt2-2",
        question:
          "Dada la recurrencia $T(n) = 3T(n/3) + n^2$, ¿cuál es su complejidad final?",
        options: [
          "$\\Theta(n)$",
          "$\\Theta(n \\log n)$",
          "$\\Theta(n^2)$",
        ...
        correctIndex: 2,
        explanation:
          "¡Muy bien! Aquí $a=3, b=3$. El costo de hojas es $n^{\\log_3 3} = n$. Dado que $f(n)=n^2$ es estrictamente mayor que $n$ en un factor polinomial de $n$, la raíz domina el tiempo (Caso 3), dando como resultado $\\Theta(n^2)$.",
        analogousExplanation:
          "Dividir en 3 es barato, pero unir esas tres partes cuesta un esfuerzo desproporcionado al cuadrado. Ese gran esfuerzo al inicio del proceso opaca todo lo demás.",
      },
      {
        id: "qt2-3",
        question:
          "¿Por qué la ecuación $T(n) = 2T(n/2) + \\frac{n}{\\log n}$ NO se resuelve directamente con el Teorema Maestro clásico?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "Correcto. El Teorema Maestro clásico exige una diferencia polinomial ($n^{\\epsilon}$). En este caso, $f(n) = n / \\log n$ es menor que $n$, pero la diferencia carece de esa separación polinomial, cayendo en un hueco del teorema.",
        analogousExplanation:
          "Es como intentar usar una regla métrica para medir células. La herramienta (Teorema Maestro) es demasiado tosca para registrar diferencias sutiles (logarítmicas), necesitas un microscopio (Método de Sustitución).",
      },
      {
        id: "qt2-4",
        question:
          "Analiza el bucle: \\`for(int i = 0; i < n; i++) { for(int j = 0; j < 1000000; j++) { x++; } }\\`. ¿Cuál es su complejidad asintótica?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Perfecto! Aunque $1,000,000$ parece enorme, asintóticamente es solo un valor constante multiplicativo $O(1)$. $\\Theta(1,000,000 \\times n)$ se simplifica a $\\Theta(n)$ según las reglas asintóticas de dominancia.",
        analogousExplanation:
          "Caminar $n$ pasos cargando un saco de 1 kilo o uno de 10 kilos sigue siendo escalar linealmente respecto a los pasos ($n$). La mochila constante no cambia la escala.",
      },
      {
        id: "qt2-5",
        question:
          "¿Qué garantiza la Condición de Regularidad ($a \\cdot f(n/b) \\le c \\cdot f(n)$) del Caso 3 del Teorema Maestro?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          'Correcto. Para afirmar que la "raíz domina" todo el árbol, no basta con ser mayor que las hojas, debe existir una constante fraccionaria $c < 1$ que compruebe que al descender en el árbol, la suma total de trabajo por nivel disminuye constantemente.',
        analogousExplanation:
          "Garantiza que la estructura piramidal sea sólida: el gerente (raíz) gana más de forma consistente que la suma del salario de todos sus subordinados directos combinados.",
      },
    ],
    exercises: [],
    prevItemId: "clase-9",
  },
  {
    ...
    durationMinutes: 45,
    summary:
      "De la búsqueda secuencial $O(n)$ a la poderosa búsqueda binaria $O(\\log n)$ en arreglos ordenados, incluyendo variantes y búsqueda por interpolación.",
...
*   **Recurso Adicional:** Intenta visualizar la búsqueda de interpolación y compárala visualmente contra la binaria en la herramienta de simulaciones visuales. Nota cómo la interpolación "salta" asimétricamente.`,
    ...
    checkQuestions: [
      {
        id: "q10-1",
        question:
          "¿Por qué la Búsqueda Binaria tiene una complejidad temporal de $O(\\log n)$?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Excelente! Si divides 1,000,000 por 2 repetidamente, llegarás a 1 en aprox. 20 pasos. Eso es el logaritmo en base 2.",
        analogousExplanation:
          'Como adivinar un número del 1 al 1000 preguntando "¿es mayor a 500?". Cada respuesta elimina la mitad de las posibilidades.',
      },
      {
        id: "q10-2",
        question:
          "¿Qué sucede si ejecutamos una Búsqueda Binaria estándar buscando el valor 15 en el arreglo desordenado `[20, 5, 15, 30, 10]`?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "Correcto. La Búsqueda Binaria descarta ciega y matemáticamente las mitades. Si el arreglo no está ordenado, su lógica deductiva falla por completo y omitirá zonas que contienen el valor.",
        analogousExplanation:
          'Si buscas la palabra "Gato" en un diccionario donde las páginas se imprimieron desordenadas, abrir por la mitad (M) y buscar a la izquierda porque G < M no te garantiza nada. "Gato" podría estar al final.',
      },
      {
        id: "q10-3",
        question:
          "En un arreglo ordenado con duplicados `[2, 4, 4, 4, 8, 10]`, deseamos encontrar la **primera** ocurrencia del 4. ¿Qué modificación clave requiere la Búsqueda Binaria?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Muy bien! Al encontrar el 4, sabemos que existe, pero podría haber un 4 previo. Acotamos el límite superior a `high = mid - 1` y seguimos buscando para encontrar la coincidencia más a la izquierda posible.",
        analogousExplanation: "",
      },
      {
        id: "q10-4",
        question:
          "¿En qué escenario la Búsqueda por Interpolación (Interpolation Search) brilla alcanzando $\\Theta(\\log \\log n)$?",
        options: [
        ...
        correctIndex: 3,
        explanation:
          "Correcto. La interpolación asume que los valores se distribuyen de forma predecible y lineal a lo largo de los índices, permitiendo saltar casi directamente a la posición estimada. Si la distribución es mala (ej. exponencial), puede degradarse a $O(n)$.",
        analogousExplanation:
          'Cuando buscas un número telefónico que empieza con "W", no abres la guía telefónica a la mitad (letra M), abres el libro en un 90% de su espesor porque sabes cómo se distribuyen.',
      },
    ],
    exercises: [],
    prevItemId: "taller-2",
  },
  {
    ...
    durationMinutes: 60,
    summary:
      "Los algoritmos de ordenamiento elementales. Lentos asintóticamente $O(n^2)$, pero vitales para entender los cimientos del diseño in-place.",
    theoryContent: `## 1. Introducción y Motivación
...
`,
    ...
    checkQuestions: [
      {
        id: "q11-1",
        question:
          'En base al análisis formal, ¿por qué se dice que el "Mejor Caso" de Insertion Sort es $\\Theta(n)$?',
        options: [
        ...
        correctIndex: 0,
        explanation:
          "¡Excelente! En un arreglo ordenado, `A[i] > key` es falso en el primer intento del `while` de Insertion Sort. El tiempo total es dominado puramente por el bucle `for` exterior, resultando en un recorrido lineal de $n$ pasos.",
        analogousExplanation:
          "Imagina que te dan las cartas ya ordenadas. Solo levantas la carta nueva, ves que ya es mayor a la de tu extremo derecho y la dejas ahí, sin desplazar nada. Cuesta muy poco trabajo.",
      },
      {
        id: "q11-2",
        question:
          '¿Qué es un "Invariante de Bucle" (Loop Invariant) según Cormen?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          "Correcto. El Invariante de Bucle se demuestra en 3 partes: Inicialización, Mantenimiento y Finalización. En el caso de Insertion Sort, el invariante afirma que el subarreglo `A[0...j-1]` siempre está ordenado con los elementos originales.",
        analogousExplanation:
          "Es como la garantía de limpieza de tu casa al barrer: Si empiezas con tu cuarto limpio (Inicialización), y al pasar al pasillo aseguras no ensuciar el cuarto (Mantenimiento), cuando termines de barrer el último espacio, sabes que toda la casa está limpia (Finalización).",
      },
      {
        id: "q11-3",
        question:
          "Si comparas el Mejor Caso de *Insertion Sort* con el Mejor Caso de *Selection Sort* sobre un arreglo ya ordenado, ¿cómo se comportan asintóticamente?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Muy bien! Selection Sort no sabe si el arreglo está ordenado. Siempre asume que el elemento actual no es el más pequeño y debe escanear *todo el resto del arreglo* para verificarlo. Insertion Sort reacciona y corta prematuramente si detecta orden.",
        analogousExplanation:
          'Selection Sort es terco: aunque el mazo ya esté ordenado, siempre mira todas y cada una de las cartas restantes "por si acaso" existe una menor. Insertion Sort se fía del trabajo ya hecho a la izquierda.',
      },
      {
        id: "q11-4",
        question:
          'En la teoría de algoritmos, ¿qué significa que un algoritmo de ordenamiento sea "Estable" (Stable)?',
        options: [
        ...
        correctIndex: 3,
        explanation:
          "¡Correcto! La estabilidad es crítica si estás ordenando objetos complejos por diferentes criterios (ej. ordenar personas por Edad, y luego por Apellido). Si el algoritmo es estable, mantendrá el orden original de la Edad dentro de los mismos Apellidos.",
        analogousExplanation:
          'Si "Juan Pérez (30)" estaba antes que "Ana Pérez (40)", un ordenamiento estable por apellido los mantendrá como "Juan Pérez, Ana Pérez". Si es inestable, podría invertir el empate y poner a Ana primero sin razón lógica.',
      },
      {
        id: "q11-5",
        question:
          "De los 3 algoritmos elementales (Inserción, Selección, Burbuja), ¿cuál de los siguientes enunciados define mejor la principal falencia de Selection Sort?",
        options: [
        ...
        correctIndex: 0,
        explanation:
          "Correcto. Selection Sort siempre efectúa $\\sim n^2/2$ comparaciones sin importar cómo ingresen los datos. Además, el intercambio (`swap`) a distancia rompe la Estabilidad.",
        analogousExplanation:
          'Selection sort destruye la estructura original para hacer grandes "saltos" (swaps) que rompen empates (inestable), y además se niega a detenerse incluso cuando el trabajo ya está terminado.',
      },
    ],
    exercises: [
      {
        ...
        description:
          "Escribe la función en C `void insertionSort(int arr[], int n)` que ordene in-place el arreglo usando el algoritmo de inserción de Cormen (Cap. 2.1).",
        ...
        explanation:
          "Insertion Sort es un algoritmo eficiente para arreglos pequeños o casi ordenados con complejidad $O(n^2)$ en el peor caso.",
      },
    ],
    prevItemId: "clase-10",
    nextItemId: "clase-12",
...

  {
    ...
    durationMinutes: 60,
    summary:
      "El poder del enfoque Divide y Vencerás. La guerra entre Mergesort y Quicksort en el diseño de sistemas de alto rendimiento y el miedo al peor caso.",
...
`,
    ...
    checkQuestions: [
      {
        id: "q12-1",
        question:
          'Tanto Mergesort como Quicksort operan bajo "Divide y Vencerás". ¿Cuál es la principal diferencia estructural entre ambos al analizar cómo hacen su trabajo?',
        options: [
        ...
        correctIndex: 0,
        explanation:
          "¡Excelente! Mergesort corta el arreglo inmediatamente y la carga computacional ocurre en la función `MERGE`. Quicksort ordena durante el particionado y luego las llamadas recursivas ensamblan todo lógicamente sin requerir un paso extra.",
        analogousExplanation:
          "Mergesort es como armar un rompecabezas ensamblando bloques progresivamente (de abajo hacia arriba). Quicksort separa las piezas por color desde el inicio (de arriba hacia abajo), y luego solo ordena los subgrupos.",
      },
      {
        id: "q12-2",
        question:
          "En sistemas limitados por Memoria RAM, ¿por qué Quicksort es preferido arquitectónicamente sobre Mergesort?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "Correcto. La principal falencia práctica de Mergesort es requerir copiar los elementos a subarreglos temporales durante el Merge ($O(n)$ extra). Quicksort hace todo usando intercambios de memoria directa (swaps), usando solo $O(\\log n)$ para las funciones recursivas.",
        analogousExplanation:
          "Para mezclar dos mazos de cartas en orden, Mergesort requiere una gran mesa auxiliar vacía (RAM). Quicksort reubica las cartas en las mismas manos sin requerir de mesas grandes.",
      },
      {
        id: "q12-3",
        question:
          "Un atacante descubre que utilizas Quicksort clásico eligiendo el último elemento como pivote. ¿Qué datos te enviará intencionalmente para causar una Denegación de Servicio (DoS) bloqueando el procesador?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Perfecto! Si envían datos ordenados, el último pivote causará un particionado que no divide el problema (ej. 4 elementos de un lado, 0 del otro). Así repetitivamente el algoritmo pasará de ser logarítmico a ser lineal, resultando en un tiempo de ejecución cuadrático $O(n^2)$.",
        analogousExplanation:
          'Si mides altura y siempre eliges como pivote a la persona más alta del grupo, nunca lograrás formar "grupos más bajos y más altos"; siempre aislarás a 1 persona, y te tomará una eternidad ordenarlos.',
      },
      {
        id: "q12-4",
        question:
          '¿Para qué sirve exactamente la estrategia "Mediana de Tres" (Median of Three) en Quicksort?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          "Correcto. Al extraer el primero, el centro y el último elemento, y usar como pivote al del medio, mitigamos los ataques con arreglos ordenados. Si el arreglo ya está ordenado, la mediana será un punto central perfecto, asegurando una división equilibrada y un comportamiento de $O(n \\log n)$.",
        analogousExplanation:
          "Es como tomar de muestra a tres personas: una muy alta, una mediana y una muy baja. Al elegir siempre al mediano, nos aseguramos de que siempre habrá gente por arriba y por debajo de él.",
      },
      {
        id: "q12-5",
        question:
          "La función nativa `std::sort` de C++ utiliza Introsort. ¿Por qué Introsort cambia a Insertion Sort cuando los arreglos a ordenar son muy pequeños (ej. < 16 elementos)?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          '¡Exacto! El "Divide y Vencerás" es ineficiente en su capa más inferior. El costo de llamar recursivamente a nuevas sub-rutinas supera el costo de un doble bucle trivial (overhead). Insertion Sort opera de forma ininterrumpida y se ejecuta instantáneamente a velocidades de silicio en bloques mínimos.',
        analogousExplanation:
          "Para cortar 1 millón de hojas de papel usas una guillotina industrial enorme (Quicksort). Pero si solo te quedan 3 hojas de papel en la mesa final, encender toda la guillotina industrial cuesta más tiempo y energía que simplemente tomar las tijeras de escritorio (Insertion Sort).",
      },
    ],
    exercises: [
      {
        ...
        description:
          "Implementa en C la rutina `void merge(int arr[], int l, int m, int r)` para combinar dos subarreglos ordenados `arr[l..m]` y `arr[m+1..r]`.",
        ...
        explanation:
          "Merge realiza la mezcla lineal en tiempo $O(n_1 + n_2)$, formando la columna vertebral de Merge Sort $O(n \\\log n)$.",
      },
    ],
    prevItemId: "clase-11",
    nextItemId: "clase-13",
...

  {
    ...
    topic:
      "Representaciones de grafos (Matriz vs Lista de Adyacencia) y recorridos BFS (Anchura) y DFS (Profundidad)",
    cormenChapter: "Capítulo 22: Algoritmos elementales de grafos",
    durationMinutes: 65,
    summary:
      "Estructuras no lineales en red: vértices $V$ y aristas $E$. Exploración por niveles (BFS) vs exploración profunda (DFS).",
...
*   **DFS:** Detección de Ciclos (encontrar Aristas de Retroceso / Back Edges que apuntan hacia un nodo Gris en pleno recorrido) y Ordenamiento Topológico en grafos dirigidos acíclicos (DAGs) organizando los tiempos finales $f[v]$.`,
    ...
    checkQuestions: [
      {
        id: "q13-1",
        question:
          "Para mapear una red social mundial (donde el número de vértices es inmenso pero la cantidad promedio de amigos por persona es pequeña), ¿cuál es la estructura de datos obligatoria para representar el grafo y por qué?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Excelente! Las redes sociales son el ejemplo clásico de Grafos Dispersos (Sparse Graphs). Usar una matriz de adyacencia $\Theta(|V|^2)$ requeriría memoria masiva inútil, la inmensa mayoría llena de ceros (personas que no se conocen).",
        analogousExplanation:
          "En vez de darle a cada humano del mundo una libreta enciclopédica con 8 billones de casillas marcando SÍ/NO, le das una libretita de bolsillo solo con los nombres de las personas que sí conoce.",
      },
      {
        id: "q13-2",
        question:
          'En el algoritmo BFS (Búsqueda en Anchura) detallado por Cormen, ¿qué estructura de datos auxiliar es la responsable algorítmica de que el proceso se ejecute por "capas concéntricas"?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Correcto! La Cola (First-In, First-Out) es el motor de BFS. Al colocar los nuevos vecinos al final de la cola y extraer el próximo a procesar siempre por el frente, BFS se obliga matemáticamente a procesar primero a los nodos más antiguos (los de distancia 'd') antes de tocar a los más nuevos (distancia 'd+1').",
        analogousExplanation:
          "Como hacer fila en el banco: el primero que llega (los vecinos inmediatos y más cercanos) es estrictamente el primero en ser atendido.",
      },
      {
        id: "q13-3",
        question:
          "Supongamos que desarrollas el código de un robot para escapar de un laberinto gigante y deseas garantizar matemáticamente encontrar el camino que requiera **la menor cantidad absoluta de pasos físicos**. ¿Qué algoritmo debes invocar?",
        options: [
          "Búsqueda Binaria.",
        ...
        correctIndex: 2,
        explanation:
          '¡Exacto! El superpoder intrínseco de BFS es que garantiza el "Camino Más Corto" (Shortest Path) en grafos donde todas las aristas pesan lo mismo (cada paso equivale a 1 unidad). Al avanzar radialmente, el momento exacto en que la onda BFS toca la salida representa matemáticamente la ruta más corta posible.',
        analogousExplanation:
          "BFS inunda el laberinto como si vertieras agua. El agua tocará la salida tomando estrictamente el camino físico más corto posible de forma natural. DFS, en cambio, exploraría ciegamente un túnel larguísimo y daría la vuelta entera al laberinto antes de encontrar la puerta.",
      },
      {
        id: "q13-4",
        question:
          'En la teoría de grafos, durante la ejecución de una Búsqueda en Profundidad (DFS), tu explorador recursivo intenta procesar un nodo vecino que el algoritmo determina que ya tiene el color "Gris" (Frontera). ¿Qué significa esto estructuralmente?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          'Correcto. Los nodos de color Gris son aquellos que actualmente están "activos" o abiertos en la pila de recursión (se han descubierto, pero aún no han finalizado). Si colisionas con un nodo que sigue Gris, significa que tu ruta de exploración dio la vuelta y se intersecó consigo misma en el pasado, formando un ciclo cerrado.',
        analogousExplanation:
          "Si vas dejando un hilo rojo deshilachándose por el laberinto (representando tus nodos Grises), y al doblar una esquina te encuentras con tu propio hilo rojo original, significa que caminaste en círculos.",
      },
      {
        id: "q13-5",
        question:
          "En la demostración analítica de la complejidad $O(|V| + |E|)$ de BFS y DFS, ¿de dónde proviene exactamente el término $O(|E|)$?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Muy bien! El término $O(|E|)$ emerge matemáticamente porque, aunque usamos bucles anidados ('while' o llamadas recursivas, y dentro de ellos un 'for each v in G.Adj[u]'), cada vértice ingresa y sale de la estructura principal solo una vez. Esto garantiza que cada arista se recorra solo 1 vez en grafos dirigidos o 2 en grafos no dirigidos. El otro término $O(|V|)$ proviene puramente del bucle inicial de blanqueamiento.",
        analogousExplanation:
          "Para pintar de Blanco a Negro todos los postes de una cerca, la tarea de vaciar todos los tarros de pintura cuesta $O(V)$, y la tarea de revisar todos los tramos de alambre que los unen a medida que caminas cuesta $O(E)$.",
      },
    ],
    exercises: [
      {
        id: "ex-13",
        title:
          "Ejercicio 13: Recorrido BFS en Grafo sobre Matriz de Adyacencia en C",
        description:
          "Implementa la función en C `int bfsContarNodosConectados(int matriz[10][10], int n, int inicio)` que realice un recorrido BFS sobre una matriz de adyacencia y retorne cuántos nodos son alcanzables desde `inicio`.",
        ...
        explanation:
          "BFS explora nivel por nivel (amplitud) usando una cola FIFO, garantizando descubrir los caminos más cortos en grafos no ponderados.",
      },
    ],
    prevItemId: "clase-12",
    nextItemId: "clase-14",
...

  {
    ...
    topic:
      "Árbol de Expansión Mínima (Kruskal, Prim) y Caminos Más Cortos (Dijkstra, Bellman-Ford)",
    cormenChapter: "Capítulos 23 y 24: Minimum Spanning Trees y Shortest Paths",
    durationMinutes: 75,
    summary:
      "Algoritmos Greedy sobre Grafos. Cómo conectar redes (Kruskal/Prim) y cómo navegar en ellas (Dijkstra/Bellman-Ford).",
...
Para grafos que contienen pesos negativos (como modelos financieros de arbitraje de divisas), debemos usar el **Algoritmo de Bellman-Ford**. Este algoritmo no usa heurística codiciosa; en cambio, relaja brutalmente absolutamente *todas* las aristas del grafo repetidas veces (exactamente $|V|-1$ iteraciones). Su complejidad es $O(|V||E|)$ (mucho más lento que Dijkstra), pero es robusto ante pesos negativos y capaz de detectar si existe un **Ciclo de Peso Negativo** (una anomalía donde puedes viajar en círculos ganando dinero infinito), en cuyo caso no existe solución.`,
    ...
    checkQuestions: [
      {
        id: "q14-1",
        question:
          "En la traza manual del algoritmo de Kruskal, tenemos las aristas ordenadas por peso: $A-B(1)$, $B-C(2)$, $A-C(3)$, $C-D(4)$. ¿Qué sucede matemáticamente cuando evaluamos la arista $A-C(3)$?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          '¡Excelente! Al procesar primero A-B(1) y B-C(2), los nodos A, B y C ya forman una única componente conexa. Cuando evaluamos A-C(3), la estructura Union-Find detectará que ya comparten "raíz" y la rechazará para evitar un triángulo (ciclo cerrado), que violaría la definición de un Árbol.',
        analogousExplanation:
          "Kruskal dice: A ya es amigo de B, y B es amigo de C. Entonces A ya tiene una forma indirecta de llegar a C. No gastes asfalto construyendo una autopista directa y redundante A-C.",
      },
      {
        id: "q14-2",
        question:
          'Ejecutando Dijkstra desde el nodo $S=0$, tenemos las distancias actuales: $d[A]=5$ y $d[B]=\\infty$. Sacamos $A$ de la Cola de Prioridad y vemos una arista $A \\rightarrow B$ con peso $w=2$. Tras aplicar la operación de "relajación", ¿cuál es el nuevo estado de $B$?',
        options: [
          "$d[B] = 2$",
        ...
        correctIndex: 1,
        explanation:
          "¡Perfecto! La relajación aplica la fórmula: $d[B] = \\min(\\infty, d[A] + w(A,B)) = \\min(\\infty, 5 + 2) = 7$. Como 7 es menor que infinito, actualizamos la distancia mínima a $B$ y anotamos que el mejor camino descubierto hasta ahora pasa por $A$.",
        analogousExplanation:
          "Llegar al aeropuerto A te costó 5 horas. El vuelo de A hacia B dura 2 horas. Por lo tanto, el nuevo costo total conocido para llegar a B es 7 horas (y en tu itinerario marcas que pasaste por A).",
      },
      {
        id: "q14-3",
        question:
          "¿Por qué el algoritmo de Dijkstra es matemáticamente incapaz de procesar correctamente grafos que contienen aristas con pesos negativos?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          '¡Exacto! Dijkstra está construido sobre la premisa de que los pesos son distancias físicas (siempre $\\ge 0$). Una vez que procesa el camino más corto conocido, asume ciegamente que cualquier otra ruta indirecta será más larga, negándose a revisar atajos engañosos que se "abaratan" más adelante gracias a números negativos.',
        analogousExplanation:
          'Dijkstra asume que conducir más kilómetros siempre gasta más gasolina. Si de pronto te encuentras una gasolinera mágica que te "regala" gasolina por recorrer un tramo largo (peso negativo), el GPS de Dijkstra nunca la tomará en cuenta porque descartó esa ruta por considerarla "demasiado larga" al inicio.',
      },
      {
        id: "q14-4",
        question:
          'Si te enfrentas a un problema de optimización financiera (como arbitraje de divisas) modelado como un grafo dirigido donde existen aristas con pesos negativos. ¿Qué algoritmo debes usar (Cap 24.1 de Cormen) y qué ocurre si hay un "ciclo de peso negativo"?',
        options: [
        ...
        correctIndex: 3,
        explanation:
          '¡Correcto! Bellman-Ford es la solución general para caminos más cortos. Es más costoso ($O(|V||E|)$) porque no usa la asunción Greedy de Dijkstra, sino que prueba todos los caminos posibles repetidas veces. Su gran fortaleza es que detecta matemáticamente la presencia de "Ciclos de Peso Negativo" (bucles infinitos).',
        analogousExplanation:
          'Un ciclo de peso negativo es como tener una máquina que, cada vez que giras una manivela, te da 10 dólares. La pregunta "¿Cuál es el camino más barato?" pierde sentido, porque la respuesta es "quedarme girando la manivela infinitamente". Bellman-Ford es el inspector que detecta que esa máquina está rota.',
      },
      {
        id: "q14-5",
        question:
          "A diferencia de Kruskal (que ordena todas las aristas globalmente), ¿cuál es la estrategia geométrica que utiliza el algoritmo de Prim para construir el Árbol de Expansión Mínima (MST)?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Perfecto! Prim es un algoritmo de crecimiento focalizado. Empieza como una semilla (un solo nodo) y mira todas sus ramas hacia el mundo exterior. Elige la rama más barata y absorbe ese nuevo nodo. Ahora, la semilla creció, y vuelve a mirar todas las fronteras de esta nueva masa conectada.",
        analogousExplanation:
          "Como una mancha de humedad en la pared. Empieza en un punto y se va expandiendo lentamente, siempre buscando avanzar por las partes del yeso que ofrezcan menor resistencia (menor peso), manteniendo toda la mancha unida.",
      },
    ],
    exercises: [
      {
        id: "ex-t3-1",
        title:
          "Ejercicio Taller 3.1: Detección de Componentes Conexas (Disjoint Set) en C",
        description:
          "Implementa la función en C `bool conectaMismaComponente(int padres[], int u, int v)` usando la búsqueda de la raíz representante en un Disjoint Set.",
        ...
        explanation:
          "Si dos nodos comparten el mismo representante raíz, pertenecen al mismo conjunto o componente conexa.",
      },
    ],
    prevItemId: "clase-13",
    nextItemId: "taller-3",
...

  {
    ...
    durationMinutes: 90,
    summary:
      "Implementación y manipulación práctica de algoritmos de grafos sobre matrices y listas de adyacencia.",
    visualizerType: "none",
  ...
  },
  {
    ...
    topic:
      "Divide y Vencerás, Programación Dinámica (Corte de Varilla, Mochila) y Algoritmos Voraces (Greedy)",
    cormenChapter:
      "Capítulos 15 (Dynamic Programming) y 16 (Greedy Algorithms)",
    durationMinutes: 70,
    summary:
      "Aprender cuándo reutilizar soluciones de subproblemas traslapados (Programación Dinámica) vs tomar decisiones óptimas locales (Voraces).",
...
`,
    checkQuestions: [
      {
        id: "q15-1",
        question:
          "Según Cormen, ¿cuál es la característica técnica clave que hace que un problema sea ideal para resolverse con Programación Dinámica (Dynamic Programming)?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Excelente! La Programación Dinámica brilla exactamente allí donde la Recursividad Ingenua fracasa: re-computar lo mismo un millón de veces. Al guardar los resultados de los subproblemas traslapados en una memoria, transformamos tiempo exponencial en tiempo polinomial.",
        analogousExplanation:
          "Si te pido calcular 50 * 200, haces el cálculo (10,000). Si 5 minutos después te vuelvo a pedir que calcules 50 * 200, no agarras la calculadora, simplemente recuerdas la respuesta (10,000). Eso es Programación Dinámica.",
      },
      {
        id: "q15-2",
        question:
          'A diferencia de la Programación Dinámica, ¿cómo actúa un algoritmo "Greedy" (Voraz) al tomar decisiones?',
        options: [
        ...
        correctIndex: 2,
        explanation:
          'Correcto. La esencia de la "Voracidad" es el compromiso inquebrantable con la decisión actual. No hay arrepentimiento, no hay revisión del pasado. Funciona increíblemente rápido ($O(n \log n)$ típicamente), pero solo es correcto si el problema exhibe la Propiedad de Elección Voraz.',
        analogousExplanation:
          "Greedy es como jugar ajedrez comiéndote inmediatamente cualquier pieza desprotegida del oponente sin pensar en sus trampas a 5 turnos en el futuro. Si el oponente no sabe poner trampas (Dijkstra, Kruskal), ganas. Si sabe (Mochila 0-1), pierdes catastróficamente.",
      },
      {
        id: "q15-3",
        question:
          'Analizando el "Problema del Corte de Varilla" (Rod Cutting), un árbol recursivo ingenuo toma tiempo $O(2^n)$. Si aplicamos Tabulación (Bottom-Up Dynamic Programming), ¿a qué complejidad de tiempo se reduce típicamente la construcción de la tabla para una varilla de tamaño $n$?',
        options: [
          "$O(\log n)$",
          "$O(n^2)$",
          "$O(1)$",
        ...
        correctIndex: 1,
        explanation:
          "¡Correcto! El método Bottom-Up llena una tabla iterando j desde 1 hasta n, y para cada tamaño j utiliza un bucle interno i desde 1 hasta j. Estos dos bucles anidados determinan un tiempo de ejecución cuadrático $O(n^2)$.",
        analogousExplanation:
          "Pasar de $O(2^n)$ a $O(n^2)$ significa que para una varilla de 50 metros, el algoritmo ingenuo tardaría décadas de cómputo (más de mil billones de pasos), mientras que la Programación Dinámica la resuelve en un milisegundo (2500 pasos).",
      },
      {
        id: "q15-4",
        question:
          'Al resolver el problema de Selección de Actividades (Activity Selection), la solución Greedy elige consistentemente la actividad que **termina primero**. ¿Qué técnica usa Cormen para demostrar que esta elección "precipitada" siempre forma parte de una solución global óptima?',
        options: [
      ...
      },
      {
        ...
        analogousExplanation:
          'Greedy prefiere meter una bola de boliche carísima en una maleta por su alto valor y dejar el resto vacío porque ninguna otra cosa grande cabe. La Programación Dinámica analiza el espacio global y dice: "Mejor no llevo la bola, sino lleno todo el espacio con dos paquetes medianos que sumados valen mucho más".',
      },
    ],
    exercises: [
      {
        ...
        description:
          "Implementa la función en C `long long fibonacciDP(int n)` utilizando un arreglo de memoria (Tabulación DP) para calcular el $n$-ésimo número de Fibonacci en tiempo $O(n)$.",
        ...
        explanation:
          "La Programación Dinámica evita recalculaciones redundantes almacenando soluciones a subproblemas superpuestos en $O(n)$ tiempo.",
      },
    ],
    prevItemId: "taller-3",
    nextItemId: "clase-16",
  },
  {
    ...
    topic:
      "Síntesis global de los algoritmos y estructuras de datos aprendidos, complejidades y casos de uso.",
    cormenChapter: "Resumen Global",
    durationMinutes: 90,
    summary:
      "Consolidación final del curso: un mapa conceptual definitivo de cuándo usar qué algoritmo y un examen integrador de todos los temas.",
    visualizerType: "none",
    ...
    checkQuestions: [
      {
        id: "q16-1",
        question:
          "Si en una entrevista técnica te piden diseñar una red de fibra óptica para conectar 1000 ciudades minimizando los kilómetros totales de cable a utilizar, ¿qué algoritmo de tu arsenal eliges?",
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Exacto! Este es el caso clásico del Árbol de Expansión Mínima (MST). Dijkstra encuentra la ruta óptima desde UN origen a UN destino, pero el MST garantiza que TODAS las ciudades estén conectadas formando un único grafo conexo con el menor gasto total de cable (aristas).",
        analogousExplanation:
          "Dijkstra es el GPS para tu auto. Kruskal es el gobierno decidiendo cómo pavimentar las carreteras gastando el mínimo de asfalto para que ningún pueblo quede aislado.",
      },
      {
        id: "q16-2",
        question:
          "El entrevistador te muestra una función recursiva que calcula los números de Fibonacci. Te das cuenta de que para $n=50$, el servidor colapsa por timeout tras minutos calculando. ¿Qué técnica aplicas inmediatamente?",
        options: [
        ...
        correctIndex: 2,
        explanation:
          "¡Perfecto! Fibonacci es el ejemplo por excelencia de Subproblemas Traslapados. La recursión ingenua recalcula subproblemas idénticos exponenciales veces. Al guardar los resultados en un array de tamaño 50, el tiempo de cálculo baja de años a microsegundos.",
        analogousExplanation:
          "Si el servidor es un oficinista, la recursión es pedirle que cuente con los dedos cada vez. La Programación Dinámica es darle una libreta para que anote el resultado y simplemente lo lea la próxima vez que se lo preguntes.",
      },
      {
        id: "q16-3",
        question:
          'Si te piden detectar fraude en transacciones bancarias (lavado de dinero), representas las cuentas como nodos y transferencias como aristas dirigidas. Descubrir un "Ciclo" demuestra el fraude. ¿Qué algoritmo de grafos usas?',
        options: [
        ...
        correctIndex: 1,
        explanation:
          "¡Excelente! DFS con coloreado de nodos (Blanco, Gris, Negro) es la herramienta matemática suprema para detectar ciclos en grafos dirigidos. Si DFS choca con un nodo que actualmente está en la pila de recursión (Gris), el ciclo está demostrado matemáticamente.",
        analogousExplanation:
          "Si vas dejando un rastro de pintura fresca (Gris) por las calles que caminas, y de repente pisas pintura fresca, significa que has caminado en círculos y volviste al punto de partida.",
      },
      {
        id: "q16-4",
        question:
          'Diseñando una base de datos, te das cuenta de que hacer un "SELECT" buscando un número de identidad (ID) en un archivo desordenado toma tiempo $O(n)$. ¿Qué proponen CLRS para reducir este tiempo drásticamente?',
        options: [
        ...
        correctIndex: 3,
        explanation:
          '¡Correcto! Esta es la base teórica de los "Índices" en Bases de Datos. En lugar de leer un millón de filas una por una (Búsqueda Lineal $O(n)$), si los IDs están ordenados, un Árbol o Búsqueda Binaria encontrará el registro en apenas ~20 pasos ($O(\log n)$).',
        analogousExplanation:
          "En vez de leer un diccionario página por página (Búsqueda Lineal), abres el diccionario por la mitad y descartas la mitad inútil repetidamente (Búsqueda Binaria/Árboles).",
      },
      {
        id: "q16-5",
        question:
          'En un sistema de mapas de GPS, las rutas de una ciudad tienen tiempos (pesos) en las calles. Hay un error en el sistema de peajes que registra un tramo con un costo de peaje "negativo". ¿Por qué fallará Dijkstra y qué algoritmo deberíamos usar en su lugar?',
        options: [
        ...
        correctIndex: 0,
        explanation:
          '¡Muy bien! El algoritmo de Dijkstra (Voraz) asume que sumar más aristas a un camino siempre aumenta su costo, por lo que una vez que finaliza un nodo, confía ciegamente en que es inmejorable. Bellman-Ford "relaja" todas las aristas múltiples veces, permitiéndole retroceder y corregir caminos si detecta que un peso negativo abarata una ruta ya visitada.',
        analogousExplanation:
          "Dijkstra nunca mira atrás; asume que avanzar cuesta dinero. Si existe un camino mágico que te devuelve dinero (peso negativo), Dijkstra nunca lo verá. Bellman-Ford revisa meticulosamente todas las rutas varias veces buscando esas ofertas.",
      },
      {
        id: "q16-6",
        question:
          "Estás diseñando la base de datos para una tienda y tienes que elegir un algoritmo de ordenamiento para ordenar cientos de miles de transacciones diariamente. Necesitas garantía estricta de tiempo (sin peores casos malos) pero tienes RAM muy limitada. ¿Cuál algoritmo seleccionas?",
        options: [
        ...
        analogousExplanation:
          "Heap Sort es como un coche todoterreno: no es el más veloz en la autopista de carreras (donde gana Quicksort), pero te asegura que llegará a la meta en un tiempo constante sin importar cuán difícil sea el terreno (peor caso), y gasta un tanque de gasolina pequeñísimo (in-place).",
      },
      {
        id: "q16-7",
        question:
          'Se requiere calcular la cantidad de "saltos" (conexiones de red) que hace un paquete de internet desde tu PC hasta un servidor en Japón, asumiendo que todos los saltos toman el mismo tiempo de tránsito. ¿Qué algoritmo te dirá la ruta con menos saltos?',
        options: [
          "Algoritmo Voraz (Greedy).",
        ...
        correctIndex: 3,
        explanation:
          '¡Perfecto! Como todos los saltos toman el mismo tiempo (el grafo "no tiene pesos" o todas las aristas pesan 1), el algoritmo óptimo para la ruta más corta es BFS. Explorar nivel por nivel garantiza que el primer camino que llegue al servidor de destino sea matemáticamente el de menos saltos.',
        analogousExplanation:
          "Si el agua se inunda a través de túneles en todas direcciones a la misma velocidad (BFS), el agua mojará primero el lugar más cercano en distancia absoluta.",
      },
      {
        id: "q16-8",
        question:
          "Tienes 1,000,000 de registros casi completamente ordenados. Solo unos 5 registros están fuera de su posición. ¿Qué algoritmo de ordenamiento completaría el trabajo casi instantáneamente (en tiempo $O(n)$)?",
        options: [
          "Heap Sort.",
          "Insertion Sort.",
          "Merge Sort.",
