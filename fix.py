...
new_theories = [
    r'''# Capítulo 1: Introducción General (El Tutorial K&R)
...
Sigue las simulaciones interactivas y prueba compilar los ejemplos en la zona de ejercicios.''',
    ...
    r'''# Capítulo 2: Tipos, Operadores y Expresiones
...
* `goto`: C proporciona la sentencia incondicional `goto label;`. Aunque fue la principal herramienta de control antes de la programación estructurada, su uso en C está fuertemente desaconsejado ("código espagueti") a menos que se trate de salir limpiamente de ciclos muy anidados en gestiones de errores, patrón usado ampliamente en el Kernel de Linux.''',
    ...
    r'''# Capítulo 4: Funciones y la Estructura del Programa
...
**Cuidado**: Un macro no es una función, es un reemplazo de texto crudo. `MAX(i++, j++)` incrementará la variable dos veces debido a su expansión sintáctica.''',
    ...
    r'''# Capítulo 5: Punteros y Arreglos
...
El operador de puntero a función permite el envío de comportamientos como argumentos, cimentando rutinas altamente escalables como la famosa función `qsort` estándar.''',
    ...
    r'''# Capítulo 6: Estructuras (Structs, Uniones y Enum)
...
En este caso, `sizeof(union Data)` será igual a 20 (el tamaño del elemento más grande). El programador tiene la responsabilidad total de recordar qué tipo fue almacenado allí por última vez. Extraer el campo como `int` si fue almacenado como `float` resultará en una lectura basura de los bits en crudo.''',
    ...
    r'''# Capítulo 7: Entrada y Salida (`stdio.h`)
...
Las funciones binarias se combinan con **`fseek(fp, desplazamiento, origen)`** que permite mover el cabezal del lector a cualquier byte del archivo sin leerlo secuencialmente de inicio a fin.''',
    ...
    r'''# Capítulo 8: La Interfaz del Sistema Operativo UNIX
...
# We need to find the locations of theoryContent and replace them.
# Given that the file is broken, let's use a regex that matches from `theoryContent: ` to `,\n    codeExamples:`
# Since there are inner backticks, we use a non-greedy match that might capture those backticks.
# Wait, it's easier to split the file by `theoryContent:` and `codeExamples:`

parts = re.split(r'theoryContent:\s*`.*?`\s*,\s*codeExamples:', content, flags=re.DOTALL)

if len(parts) != 9:
    print(f"Error: Found {len(parts) - 1} matches instead of 8")
    import sys
    sys.exit(1)

new_content = parts[0]
for i in range(8):
    theory = new_theories[i]
    # We must properly escape backticks inside the typescript template string!
    # And dollar signs just in case
    theory_escaped = theory.replace('\\', '\\\\').replace('`', '\\`').replace('$', '\\$')
    
    new_content += f"theoryContent: `{theory_escaped}`,\n    codeExamples:"
    new_content += parts[i+1]

with open('src/data/cCourseData.ts', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Replaced successfully and escaped backticks.")
